...
\centering
  \resizebox{0.45  \textwidth}{!} {                                                                                                                                             % resize figure if you want
    \begin{tikzpicture}  [every edge quotes/.append style = {anchor=south, sloped}]
       \draw[thick, ->] (-0.35,0) -- (6,0) coordinate [label={[xshift=0.30cm, below]  $t$}] (t);                                                       % xshift -- get some extra space
       \draw[thick, ->] (0,-0.35) -- (0,6) coordinate [label={[xshift=-0.30cm, left]  $x$}] (x);                                                         % xshift -- get some extra space
        \coordinate[label=below left:$ O $] (O);                                                                                                                           % save origin (O) to calculate \theta below                                                
        \draw[draw=none] (0,4) node[left] {$x_2$} -| (4,0) node[below] {$t_2$};
        \draw[draw=none] (0,2) node[left] {$x_1$};                                                                                 % draw dashed lines to a and b
         \draw[draw=none] (1.75,0) node[below] {$t_1$};    
         \fill (0,4) circle(0.05);      
         \fill (0,2) circle(0.05);                                                                                                                                                   % draw a dot on the y axis at b (could do this by declaring coordinates)
         \fill (4,0) circle(0.05); 
         \fill (1.75,0) circle(0.05);  
        \end{tikzpicture}
  }                                                                                                                                                                                             % end resizebox                                                                                           
 \caption{experiment}
