...
\centering
  \resizebox{0.80 \textwidth}{!} {												% resize figure if you want
   \begin{tikzpicture}[black!75,thick]
        \draw[thick, ->] (0,0) -- (8,0) coordinate [label={[right] $x$}];		% draw x axis     
        \draw[ultra thick] (0,0) -- (0,5);										% draw vertical spring support
...
%        
      ...
      \draw[dashed] (c1) -- (c2) coordinate [label={[above] \text{{\footnotesize \underline{Equilibrium Point}}}}];   
      \fill (c1) circle(0.05);  
      \path (c1) node[yshift=-0.5mm, below] {$0$};
      \draw[latex-] (3.50,1.20) -- (5.50,1.20) coordinate [label={[font=\footnotesize,color=black,midway,above] {$\vec{F} = -kx\hat{i}$}}]; 
      \draw[dashed] (5.50,1.20) -- (5.50,0.00);
      \fill (5.50,1.20) circle(0.05);            
      \fill (3.50,1.20) circle(0.05);
%
...
%
        \draw [
              decoration={
                 ...
                 pre length=3mm,
                 post length=3mm},
             decorate] (0,2.50) -- (3.5,2.50);
        \draw [draw=none] (0,2.50) -- (3.5,2.50) coordinate [label={[midway,above, yshift=0.25cm] \text{{\tiny {\bf Equilibrium State}}}}];  
        \fill (0,2.50) circle(0.05);											% draw a dot connecting the spring to the y axis
        \fill (3.5,2.50) circle(0.05);											% draw a dot connecting the spring to the mass
 ...
 %
       \node[draw,
	     ...
	     minimum height=0.75cm,
	     anchor=north,
	     label=center:$m$] at (4.00, 2.90) {};
	   \fill (3.50,2.50) circle(0.05);											% draw a dot connecting the spring to the mass
    \end{tikzpicture}
  }																				% end resizebox                                                                                           
