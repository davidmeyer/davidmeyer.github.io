...
\setlength {\fboxrule}{0.35pt}					% set the outside fcolorbox line thickness
%
%	Draw the picture
%
\begin{figure}[H]
  ...
  %\fcolorbox{blue}{white}{
     %\resizebox{0.8\textwidth}{!}{%
        \begin{tikzpicture}
           ...
           \draw[->] (FA) edge (FB)							% draw the horizontal edges
                     (GA) edge (GB); 
           \draw[]   (FA) -- (FB) node [midway,above,scale=0.75] {$F(f)$};
           \draw[]   (GA) -- (GB) node [midway,below,scale=0.75] {$G(f)$};

           \draw[->] (FA) edge (GA)							% draw the vertical edges
                     (FB) edge (GB);
           \draw[]   (FA) -- (GA) node [midway,left,scale=0.75]  {$\eta_{A}$};
           \draw[]   (FB) -- (GB) node [midway,right,scale=0.75] {$\eta_{B}$};
        \end{tikzpicture}
  %  }													    % end resizebox
 %  }														% end fcolorbox
 \label{figure:natural_transformation}
 \caption{This would be better if you could put a box around it and resize it}
\end{figure}
...
%   
         ...
         \draw[]   (Ffl) -- (Ffr) node [midway,above,scale=0.75] {$F(f)$};
     
         \draw[->] (FB) -- (GB) node [midway, right,scale=0.75] {$\eta_{B}$};
         \draw[]   (FB) -- (GB) node [below,xshift=0.5mm] {$G(B)$};
          
         \draw[->] (Gfl) -- (Gfr) node [midway,below,scale=0.75] {$G(f)$};   
       \end{tikzpicture}
    }														% end resizebox
   }														% end fcolorbox
\label{figure:natural_transformation}
