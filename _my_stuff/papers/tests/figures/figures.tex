...
\section{Single Winner Case}
\begin{enumerate}
\item Consider the single winner case when the jackpot amount is 
$\$1.15 \times 10^9$. The amount the winner would receive varies 
based on whether the jackpot is paid as a lump sum or annuity, 
and after taxes:

\begin{itemize}
\item Lump sum payout is typically about 60\% of the advertised jackpot

\item We also need to subtract Federal taxes, which are in the 24-37\% range
\end{itemize}

\noindent
Then for a single winner of a \$1.15B  jackpot:

\begin{itemize}
\item The lump sum gives you something like 60\% of the jackpot


\begin{itemize}
\item So assume that the lump sum is $(1.15 \times 10^{9}) \times 0.60 = \$690,000,000$ 
\end{itemize}

\item Federal tax is in the 24-37\% range, so for simplicity
assume a tax rate of 30\%

\begin{itemize}
\item Then after taxes we have $\$690,000,000 \times (1.00-0.30) = \$483,000,000$
\end{itemize}
\end{itemize}

\noindent
So we'll use \$483,000,000  as the effective jackpot.

\item Now we can calculate the expected value of the jackpot,
$\text{EV}_{\text{jackpot}}$ as follows:

\begin{equation*}
\text{EV}_{\text{jackpot}} = \dfrac{1}{302,575,350} \times \$483,000,000 \approx \$1.60
\end{equation*}

\item The expected value of a \$2 ticket (for the jackpot only) is approximately 
\$1.60 when the jackpot is $\$1.15 \times 10^{9}$ and there is a single winner.
\end{enumerate}

\bigskip
\noindent
{\bf Summary}: For a \$1.15 billion jackpot, the jackpot-only expected value of 
a \$2 Mega Millions ticket when there is a single winner is approximately \$1.60 
before considering the cost of a ticket. After subtracting the \$2 ticket cost, 
the net expected value is apparently $\color{red}{(0.40)}$, or -40 cents.

\newpage
\section{Multiple Winners Case}
These are the key values for this case:

\begin{enumerate}
\item Advertised Jackpot: $\$1.15 \times 10^9$
\item Lump Sum Option: Typically ~60\% of the advertised amount, so take \\
      Lump sum = $(\$1.15 \times 10^9) \times 0.60 = \$690 \times 10^{6}$
\item Taxes: Both Federal and potentially state taxes apply
\begin{itemize}
\item Federal tax rate: ~37\%
\item State tax rates vary but average around 5\%
\item Combined tax rate: ~42\%
\item Post-tax lump sum = $\$690 \times 10^{6} \times (1.00-0.42) = \$400.2 \times 10^6$.
\end{itemize}
\end{enumerate}
...
\medskip
\begin{equation*}
\text{Post-Tax Lump Sum Share} = \dfrac{\text{Post-Tax Lump Sum}}{n} = \dfrac{\$400.2 \times 10^6}{n}
\end{equation*}
...
\begin{itemize}
\item $n=1$: $\$400.2 \times 10^6$
\item $n=2$: $\$200.1 \times 10^6$
\item $n=3$: $\$133.4 \times 10^6$
\end{itemize}

\subsection{Expected Number of Winners}
The expected number of winners depends on $T$,
the number of tickets sold, and the odds of 
winning as follows

\begin{equation*}
\text{Expected Number of Winners} = T \times \dfrac{1}{302,575,350}
\end{equation*}

\medskip
\noindent
Suppose 500 million tickets are sold, so that $T = 500 \times 10^6$. 
Then we have

\medskip
\begin{equation*}
\text{Expected Number of Winners} = (500\times 10^6) \times \dfrac{1}{302,575,350} = 1.65
\end{equation*}


\subsection{Post-Tax Lump Sum per Winner}
If there are 1.65 winners on average, each winner receives:

\begin{equation*}
\text{Post-Tax Lump Sum Share} = \dfrac{\$400.2 \times 10^6}{1.65} = \$242.5 \times 10^6
\end{equation*}

\subsection{Expected Jackpot Value per Ticket}
The expected jackpot payout per ticket is:

\medskip
\begin{equation*}
\text{EV}_{\text{jackpot}} = \text{Post-Tax Lump Sum Share} \times \text{Odds of Winning}
\end{equation*}

\medskip
\noindent
So $\text{EV}_{\text{jackpot}}$ is

\medskip
\begin{equation*}
\text{EV}_{\text{jackpot}} = (\$242.5 \times 10^6) \times \dfrac{1}{302,575,350} \approx \$0.80
\end{equation*}

\bigskip
\subsection{Net $\text{EV}_{\text{jackpot}}$}

\medskip
\begin{equation*}
\text{EV}_{\text{jackpot}} = 0.80 - 2 = -\$1.20
\end{equation*}

\bigskip
\noindent
{\bf Summary}: For a $\$1.15 \times 10^9$ jackpot with $500 \times 10^6$ tickets sold, 
the expected value of the jackpot portion only is approximately \$0.80. Subtracting 
the ticket cost, the net EV is approximately $\color{red}{(1.20)}$,
meaning the ticket is not profitable in this scenario.
...
\begin{figure}[H]										% build the figure
  ...
  \fcolorbox{blue}{white} {								% make the frame blue
...
\begin{figure}
 \center{\includegraphics[frame, scale=0.50] {images/schrodingers_cat_cartoon.png}}
...
%	\usepackage{halloweenmath}
%
%	Note: Here the \resizebox is inside the \fcolorbox{orange}{white}. You could also put 
%	the \resizebox outside the \fcolorbox (either one). 
%
%
%	Start by setting space and frame thickness of the \fcolorbox(es)
%
%
\setlength {\fboxsep} {1.00em}					% get more space between \fcolorbox and content
\setlength {\fboxrule}{1.25pt}					% set the outside \fcolorbox line thickness
%												%
%												%
%												%
\begin{figure}[H]								% build the figure
  ...
  \fcolorbox{black}{white} {					% make the outside frame black
    \setlength {\fboxrule}{1.00pt}				% set the inside \fcolorbox line thickness
    \hspace{-0.60em}							% too much space on the left for some reason
    \fcolorbox{orange}{white}{					% make the inside frame orange					
      \resizebox{0.75 \textwidth}{!} {			% resize the inside of the \fcolorbox{orange}{white}
        \hspace{-0.75em}						% get rid of some space of the left
        ...
        \hspace{-0.75em}						% get rid of some space on the right				
      }											% end \resizebox
