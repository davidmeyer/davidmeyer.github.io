...
%  
			...
			\draw [->] (10.50,0.75) -- (16.5,0.85) node [midway,above,yshift=0.50cm] {$\color{red}{\text{\bf Induced topology}}$};
			\draw [draw=none] (10,0.5) -- (16.5,0.75) node [midway,above,xshift=0.50cm,yshift=-3.750cm] {$\boldsymbol{\subseteq}$};
			\draw [->] (5,-0.75) -- (16.5,-0.85);
	    ...
	    \begin{math}
          \begin{array}{llllll}
            ...
            && {\bf TS}  &--& \text{Topological Spaces}	  & (X, {\boldsymbol \tau_d)} = (X,\{B_{d,\epsilon}({\bf u}) \}) 
            												\text{, where }U \subseteq X \text{ and } (\forall {\bf u} \in U, 
															\; \exists \epsilon > 0) \text{ : } B_{d,\epsilon}({\bf u}) \subseteq U
          \end{array}
        \end{math}
     }
  }
