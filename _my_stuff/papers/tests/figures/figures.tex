...
%
       ...
       \draw[]  (1.50,0)              node[above] {$\lambda[1,0]$};
...
%	Draw the y = x line
%
       \draw [-] (-1.00,-1.00) -- (1.25,1.25) node[above] {$\lambda[x,1] = \lambda [\cos \theta, \sin \theta]$};
%
%	Intersection of y = 1 and y = x
%
       \fill [black] (1,1) circle (0.75pt);
...
%
      \fill [black] (0,0) circle (0.75pt);
%
%	Draw a few coordinates of interest
% 
      \draw (1,0)  node[below] {$(1,0)$};
      \draw (-1,0) node[below] {$(-1,0)$};
      \draw (0,1)  node [below,yshift=-0.20cm] {$(0,1)$};
...
\begin{figure}[H]
 ...
 \resizebox{\scalefactor \textwidth}{!} {
   \begin{tikzpicture}[scale=3]
     ...
     \node[below,yshift=-0.20cm] at (0,-1) {$-\infty,\frac{\pi}{2}; \infty,\frac{\pi}{2}$};
   \end{tikzpicture}
  }
 \caption{The Real Projective Line is a Circle}
