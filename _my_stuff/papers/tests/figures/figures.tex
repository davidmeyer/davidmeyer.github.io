...
%	Draw the boxes
%
        \node [draw] (sunrise) {Sunrise Plaza};
        \node[draw,right=of sunrise,xshift=2.25cm]       (hjm_trust) {HJM Trust};
        \node[draw,below left=of sunrise,xshift=1.0cm]   (magnolia) {Magnolia};
        \node[draw,below=of sunrise,xshift=1.5cm]        (bobier_vista) {Bobier Vista};
        \node[draw,below right=of sunrise,xshift=2.25cm] (baseline) {Baseline};
        \node[draw,below=of magnolia]                    (x) {Avalon or Baseline};
%
%	Draw the edges with (tiny) labels 
%
        \draw[->] (hjm_trust)    -- node[midway, above]              {\tiny 32,936.16}  (sunrise);
        \draw[<-] (sunrise)      -- node[midway, left,xshift=-1.0mm] {\tiny 32,936.16}  (magnolia);
        \draw[<-] (sunrise)      -- node[midway, right,xshift=1.0mm] {\tiny 195,000.00} (bobier_vista);
        \draw[->] (bobier_vista) -- node[midway, above]              {\tiny 480,000.00} (baseline);
        \draw[->] (magnolia)     -- node[midway, above]              {\tiny 15,000.00}  (bobier_vista);
        \draw[->] (x)            -- node[midway,right]               {\tiny 15,600.00}  (magnolia);
     \end{tikzpicture}
    }																			% fcolorbox
  }                                                                             % resizebox
  \caption{HJM Trust Loan Graph}
  \label{figure:loan_graph}
 \end{figure}
