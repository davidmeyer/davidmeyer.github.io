...
%
%
       \draw [thick,-latex] (O) -- (T) node [above] {$\mathbf{y}$};					% top vector
       \draw [thick,-latex] (O) -- (B) node [below] {$\mathbf{x}$};					% bottom vector
       
      \pic [draw,-latex,angle radius=11mm,angle eccentricity=1.2, "${
      	\hspace{9em} \theta = \arccos \left(\dfrac{\mathbf{x}\cdot \mathbf{y}}
		{\norm{\mathbf{x}} \norm{\mathbf{y}}} \right)}$"] 
		{angle = BM--O--TM};														% this is the angle
%
%	done
%       
   \end{tikzpicture}
