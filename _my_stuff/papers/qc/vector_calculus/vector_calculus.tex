...
Instead, note that the region consists of two pairs of parallel sides and 
so we can find the equation for each side:
...
shown Figure \ref{fig:transformation_uv_plane}. Note that this region
(the rectangle)is completely below the $u$ axis and so the area (or 
...
%	draw parallelogram
%
% \draw [ultra thick] (xy1) -- (xy2) -- (xy3) -- (xy4) -- cycle;
%
       \draw [ultra thick, fill=gray!30] (xy1) -- (O) -- (xy5) -- (xy4) -- cycle;
...
\bigskip
\noindent
Now need to solve for $x$ and $y$ in 
our system. We have expressions for
$u$ and $v$ and so can solve
for $x$ and $y$:
...
\bigskip
\noindent
Substituting back into the original double integral 
we get
...
&=& {\displaystyle \int\limits_{-10}^{0} \int\limits_{0}^{10} \left ( \left (  \dfrac{1}{5}u - \dfrac{2}{5}v \right )
 ...
 \left ( \dfrac{1}{10} \right ) du \, dv
			&\mathrel{\#} \text{\small Equations \ref{eqn:transformation_x} and \ref{eqn:transformation_y}}, \, 
			J(u,v) = \frac{1}{10} \\
[20pt]
...
&\Rightarrow& {\displaystyle \Bigg | \iint\limits_{R} (x - 2y) \, dA} \Bigg | = 10
			&\mathrel{\#} \text{\small the absolute value of the volume is 10} \\
...
region in the $uv$-plane\footnote{Recall that the region in the
$uv$-plane was a rectangle where $- \leq u \leq 10$ and $-10 \leq
v \leq 0$; see Figure \ref{fig:transformation_uv_plane}.} has an
area of $100 \text{ units}^2$.  That is, the region in the
$uv$-plane is 10 times as large as the region in the
$xy$-plane. The Jacobian $J(u,v) = \frac{1}{10}$ scales this
change in the underlying area.
