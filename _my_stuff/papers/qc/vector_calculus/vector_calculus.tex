...
where $\mathbf{a}^{\intercal}$ is the transpose of the vector 
$\mathbf{a}$. Similarly, if $\mathbf{a}$ and $\mathbf{b}$ are 
row vectors then the dot product can be written as
$\mathbf{a} \cdot \mathbf{b} = \mathbf{a}\mathbf{b}^{\intercal}$.

\bigskip
\noindent
The matrix shape of the dot product of two $n \times 1$ 
(column) vectors $\mathbf{a}$ and $\mathbf{b}$ is 


\begin{flalign*}
\mathbf{a} \cdot \mathbf{b} = \mathbf{a}^{\intercal}\mathbf{b}   
\rightarrow \underbrace{\begin{bmatrix} \hdots \hdots \hdots \end{bmatrix}}_{1 \times n}  
...
\bigskip
\begin{theorem} {\bf Cauchy–Schwarz Inequality:} $\big | \langle \mathbf{a},\mathbf{b} \rangle \big | \leq \| \mathbf{a} \| \| \mathbf{b} \|$.
...
\section{Appendix A}
I had thought that one way to prove the Cauchy–Schwarz inequality is to use the 
reverse triangle inequality, but Dave Neary pointed out that I didn't need it. I'm 
leaving the derivation of the reverse triangle inequality in this appendix
 but it's not needed.
