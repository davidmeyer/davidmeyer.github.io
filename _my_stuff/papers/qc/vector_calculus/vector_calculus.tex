...
\bigskip
\noindent
The projection of $\mathbf{a}$ onto the line generated by
$\mathbf{b}$ is $\alpha \mathbf{v}_1$. To determine $\alpha$ we
take the inner product of $\mathbf{b}$ and Equation
(\ref{eqn:alpha_beta}) as follows:
...
&=& \alpha \langle \mathbf{v}_1, \mathbf{b}\rangle + \beta \, 0
					...
					\text{ and }
					\mathbf{v}_1 = \mathbf{\hat{b}} 
					\text{ and so }  \langle \mathbf{b}, \mathbf{v}_2  \rangle
					= \langle \mathbf{v}_2, \mathbf{b} \rangle
					= 0 \\
[10pt]
...
parallel to ("in the direction of") the vector $\mathbf{b}$. 
More generally, since $\dfrac{1}{\|\mathbf{b} \|}$ 
is a scalar and scalar multiplication stretches a vector but 
doesn't change its direction (other than to maybe reverse 
it), we see that $\langle \mathbf{a}, \mathbf{b} \rangle$ 
is also the component of $\mathbf{a}$ in the $\mathbf{b}$
direction.
...
\caption{The Right Hand Rule for two Vectors $\mathbf{a}$ and $\mathbf{b}$ \cite{wiki:right_hand_rule_for_cross_products}}
...
both of the inputs is the zero vector, (either $\mathbf{a} =
\mathbf{0}$ and/or $\mathbf{b} = \mathbf{0}$) or the vectors are
parallel or anti-parallel (the angle between them is 
$\theta = 0$ or $\theta = \pi$). 
A corollary is that since the angle
between a vector and itself is $0$, the cross product of a vector
with itself is the zero vector  (by Equation (\ref{eqn:cross_product})).
That is, $\mathbf{a} \times \mathbf{a} 
...
That is, when viewed from the top or $\mathbf{z}$ 
...
\bigskip
\noindent
This determinant can be computed using Sarrus's rule
...
\bigskip
\noindent
The cross product has the following additional properties:
