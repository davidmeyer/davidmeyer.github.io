...
\usepackage{bm}															% get bold math symbols
...
{\setstretch{1.25}
\noindent 
{\bf Proof.} Assume that $\mathbf{x}$ and $\mathbf{y}$ are two linearly
independent vectors in $\mathbb{R}^3$ and that $M$ is the plane
spanned by them. Then these two vectors generate a triangle in
$M$ with sides of length $\norm{\mathbf{x}}$, $\norm{\mathbf{y}}$,
and $\norm{\mathbf{x} - \mathbf{y}}$. If $\theta \in \{0, \pi \}$
is the angle between $\mathbf{x}$ and $\mathbf{y}$ in $M$ then 
by the Law of Cosines \cite{law_of_cosines} we have
\par}
