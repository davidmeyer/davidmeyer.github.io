...
\smallskip
\noindent
Note that since $a_{j}b_{j} = b_{j}a_{j}$ ($a_j$ and $b_j$ 
are scalars and scalar multiplication is commutative\footnote{Note that not 
all dot products are commutative. For example, for the \emph{divergence} of 
$\mathbf{A}$, $\text{div}(\mathbf{A})$, we have $\nabla \cdot \mathbf{A} 
\neq \mathbf{A} \cdot \nabla$.}) we have
