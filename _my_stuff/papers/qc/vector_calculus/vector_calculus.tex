...
classes in a classification problem.} The $\mathbf{e}_i$ have
...
\bigskip
\noindent
If $\mathbf{a}$ and $\mathbf{b}$ are column vectors then 
the dot product can also be written as a matrix product:
$\mathbf{a} \cdot \mathbf{b} = \mathbf{a}^{\intercal}\mathbf{b}$
where $\mathbf{v}^{\intercal}$ is the transpose of some vector 
$\mathbf{v}$. Similarly, if $\mathbf{a}$ and $\mathbf{b}$ are 
row vectors then the dot product can be written as
$\mathbf{a} \cdot \mathbf{b} = \mathbf{a}\mathbf{b}^{\intercal}$.
