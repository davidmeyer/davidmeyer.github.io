...
the curve $C$ is the circle of radius $r$ in the first quadrant, the limits of integration 
are $a = 0$ and $b = \frac{\pi}{2}$ (that is, $t \in [ 0,\frac{\pi}{2} ] $). Putting this all together we get

\medskip
\begin{equation*}
 \int_{C} f(x,y) \, ds = \int_{0}^{\frac{\pi}{2}} f(h(t),g(t)) \, ds = \int_{0}^{\frac{\pi}{2}} f(r\cos (t), r\sin(t)) \, ds 
\end{equation*}

\bigskip
\noindent
Ok, but what is $ds$? As we saw above $ds$ is the infinitesimal  arc length
and that the Pythagorean theorem tells us that $ds = \sqrt{dx^2 + dy^2}$ (Figure 
...
\bigskip
\begin{equation*}
\int_{C} f(x,y) \, ds = \int_{a}^{b} f(g(t),h(t))  \sqrt{g^{\prime}(t)^2 + h^{\prime}(t)^2} \cdot dt	
\end{equation*}

\bigskip
\noindent
At this point our line integral has been reduced a definite integral over one variable (which hopefully 
we can solve).
