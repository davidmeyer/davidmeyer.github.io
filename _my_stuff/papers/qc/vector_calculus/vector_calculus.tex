...
The first step in solving a line integral is to find a
parameterization for the curve $C$. We would like to find a
parameterization such that the integral reduces to an integral
over a single variable.  So the parametric form of $x$ will be
$x(t) = g(h)$, and the parametric form of $y$ will be $y(t) =
h(t)$, where the parameter $t \in [a,b]$. That is


\begin{equation*}
 \int_{C} f(x,y) \, ds = \int_{a}^{b} f(h(t),g(t)) \, ds 
\end{equation*}
...
\bigskip
\noindent
Now we can write our line integral in terms of a single
...
In this case the parameterization is $g(t) = r \cos (t)$ and 
$h(t) = r \sin (t)$.  Solving for $\sqrt{g^{\prime}(t)^2 + h^{\prime}(t)^2}$ 
we get
...
\bigskip
\noindent
So in our example $ds = r \, dt$.

\bigskip
\noindent
Next we need to specify the limits of integration for our
parameterization. Since the curve $C$ is a circle in the first
quadrant our parameter $t \in [ 0,\frac{\pi}{2} ] $, so our the
