...
\bigskip
\noindent
If we replace $\mathbf{a}$ by $-\mathbf{a}$ by 
Equation (\ref{eqn:magnitude}) $\|-\mathbf{a}\| = \| \mathbf{a}\|$. 
Further, by the scalar multiplication rule for dot products 
with $c_1 = -1$ and $c_2 = 1$ we have $\langle -\mathbf{a},\mathbf{b} \rangle 
= - \langle \mathbf{a},\mathbf{b} \rangle$. So we can also see 
