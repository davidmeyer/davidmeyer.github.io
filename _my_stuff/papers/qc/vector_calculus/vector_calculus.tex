...
\bigskip
\noindent
For example $\mathbf{\hat{i}}$, $\mathbf{\hat{j}}$,
and $\mathbf{\hat{i}}$ denote
the unit vectors in the $x$, $y$, and $z$ directions
respectively. Using these definitions we can define
...
functions. Note that $\mathbf{\hat{i}}$, $\mathbf{\hat{j}}$, 
and $\mathbf{\hat{k}}$ are basis vectors in $\mathbb{R}^3$ 
...
\bigskip
\noindent
Another common notation for vectors is $\vec{r}(t)$, where $\vec{r}(t)$ 
is defined to be
