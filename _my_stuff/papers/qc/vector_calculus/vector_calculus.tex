...
$\mathbf{y} = \mathbf{\hat{b}}$). Then we can see by the triangle inequality 
...
\bigskip
\noindent
If we replace $\mathbf{a}$ by $-\mathbf{a}$ and since by 
Equation (\ref{eqn:magnitude}) we have $\|-\mathbf{a}\| 
= \| \mathbf{a}\|$ and by the scalar multiplication rule for dot products 
we have $\langle -\mathbf{a},\mathbf{b} \rangle = - \langle \mathbf{a},\mathbf{b} \rangle$ 
we can see that\footnote{There is nothing special about using $\mathbf{a}$ 
