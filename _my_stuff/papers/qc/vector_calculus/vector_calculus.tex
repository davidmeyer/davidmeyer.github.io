...
\begin{definition}A \emph{field} is an algebraic structure consisting 
of a non-empty set $\mathbb{K}$ equipped with two binary operations, $+$ 
(addition) and $\cdot$ (multiplication), satisfying the conditions:

\medskip
\begin{itemize}
\item [] (A) $(\mathbb{K},+)$ is an Abelian group\footnote{See Appendices A and B 
             for more on fields and Abelian groups.} with identity element $0$ 
             (called zero).
...
\bigskip
\begin{definition}
{\bf Dot Product:} $\langle \mathbf{a}, \mathbf{b} \rangle$
...
of the dot product (as defined in Equation (\ref{eqn:dot})) but does not commute. 
That is
...
\noindent
Here's a small theorem that relates the algebraic properties of 
the dot product to its geometric properties: 
...
$\mathbb{R}^3$ and that $M$ is the plane spanned by them. Then these 
two vectors generate a triangle in $M$ with sides of length 
...
&=& {\norm{\mathbf{x}}}^2 - 2  \langle \mathbf{x} , \mathbf{y} \rangle + \; {\norm{\mathbf{y}}}^2 
 		&\hspace{2em} \mathrel{\#} \text{${\norm{\mathbf{x}}}^2 = \sum\limits_{j = 1}^{n} {x}^{2}_{j}$ 
					...
					(Equations (\ref{eqn:magnitude}) and (\ref{eqn:dot}))} \\
[11pt]
...
\bigskip
\noindent
Setting this last expression for ${\norm{\mathbf{x} - \mathbf{y}}}^2$
equal to the right-hand side (RHS) of Equation (\ref{eqn:law_of_cosines}) 
we get
...
of this equation and dividing by $-2$ gives us
...
\bigskip
\noindent
This is geometric interpretation of the dot product
is depicted in Figure \ref{fig:geometric_dot_product}.
...
%
%
		\draw [line width=0.35mm,-latex] (O) -- (T) node [above] {$\mathbf{y}$};		% draw y (top vector)
		\draw [line width=0.35mm,-latex] (O) -- (B) node [below] {$\mathbf{x}$};		% draw x (bottom vector)
%
%
%       
		\pic [draw,line width=0.25mm,-latex,angle radius=10mm,angle eccentricity=1.2]	% could do \pic [...,"${...}$"] {angle = ...}; 
			{angle = BM--O--TM};														% this draws the angle
		\fill [black] (O) circle (0.05) node[below,left,xshift=-0.10cm] {$O$};			% label the origin
		\node [yshift=0.25mm,xshift=6.0mm] at (O) {$\theta$};							% label theta 		
		\node[below,left,xshift=9.0cm,draw,rectangle,color=black,thin,scale=1] at (O)	% get \theta = ... in the right place
...
\bigskip
\begin{figure}[H]
\center{\includegraphics[scale=0.38] 
...
&=& \langle \mathbf{b}, \alpha \mathbf{v}_1 \rangle + \langle \mathbf{b}, \beta \mathbf{v}_2 \rangle
					&\hspace{3em} \mathrel{\#} \text{the dot product is distributive 
...
&\leq& {\norm{\mathbf{x}}}^2 - 2 \langle \mathbf{x},\mathbf{y} \rangle + \; {\norm{\mathbf{y}}}^2
					...
					and $\langle \mathbf{x},\mathbf{y} \rangle  = \sum\limits_{j = 1}^{n} x_{j} y_{j}$ 
					(Equations  (\ref{eqn:magnitude}) and (\ref{eqn:dot}) respectively)} \\
[12pt]
...
Further, by the scalar multiplication rule for dot
