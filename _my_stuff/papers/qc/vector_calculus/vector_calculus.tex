...
The first step in solving a line integral is to find a parameterization
...
circle in the first quadrant. That is, our curve $C$ is
$x^2 + y^2 = r^2$ and so the parametric form 
of $x$ and $y$ would be  $x(t) = g(t) = r \cos (t)$ and $y(t) = h(t) = r \sin (t)$.

\bigskip
\noindent
We also need to specify the limits of integration for our parameterization. Since
the curve $C$ is a circle in the first quadrant our parameter $t \in [ 0,\frac{\pi}{2} ] $, 
so our the limits of integration are $a = 0$ and $b = \frac{\pi}{2}$. Putting this all 
together we get

\medskip
\begin{equation*}
 \int_{C} f(x,y) \, ds = \int_{a}^{b} f(h(t),g(t)) \, ds = \int_{0}^{\frac{\pi}{2}} f(r\cos (t), r\sin(t)) \, ds 
\end{equation*}

\bigskip
\noindent
All good, but what is $ds$? We saw in Section \ref{sec:arc_length} that $ds$ is the infinitesimal 
arc length and that by the Pythagorean theorem $ds = \sqrt{dx^2 + dy^2}$ (Figure 
\ref{fig:pythagorean_theorem}). Using this expression for $ds$ we can see that
...
&=& \sqrt{dg^2 + dh^2} \; \dfrac{dt}{dt}
			...
			$1 = \dfrac{dt}{dt}, \; dt > 0$}  \\
[15pt]
&=& \sqrt{dg^2 + dh^2} \;  \Bigg [\sqrt{\dfrac{1}{dt^2}} \;  dt \Bigg ]
			&\qquad \qquad \mathrel{\#} \dfrac{x}{x} 
			= \sqrt{\left (\dfrac{1}{x} \right)^2} \;  x 
			= \sqrt{\dfrac{1}{x^2}} \;  x, \; x > 0  \\
[15pt]
&=& \sqrt{\Big ( dg^2 + dh^2 \Big ) \;  \left (\dfrac{1}{dt^2} \right)} \; dt
			&\qquad \qquad \mathrel{\#} \text{simplify} \\
[15pt]
...
\bigskip
\noindent
Now we can rewrite our line integral in terms of a single variable $t$ as follows:
...
\bigskip
\noindent
At this point our line integral has been reduced a definite integral over one 
variable (which hopefully we can solve).
