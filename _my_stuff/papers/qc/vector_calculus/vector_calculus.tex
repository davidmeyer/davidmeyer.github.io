...
\bigskip
\noindent
Similarly, $\mathbf{\hat{i}}$, $\mathbf{\hat{j}}$, and $\mathbf{\hat{i}}$ 
are unit vectors in $\mathbb{R}^3$ in the $x$, $y$, and $z$ 
directions respectively. Note that $\mathbf{\hat{i}}$, $\mathbf{\hat{j}}$, 
and $\mathbf{\hat{k}}$ are the canonical basis vectors for $\mathbb{R}^3$ 
...
\bigskip
\noindent
I have also seen $\mathbf{e}_i$ used to represent the $i^{th}$ basis 
vector in $\mathbb{R}^n$. So $\mathbf{e}_i$ is a vector with a one 
in the $i^{th}$ position and a zero in each of the other $n - 1$ positions.
In $\mathbb{R}^3$ this means that  
$\mathbf{e}_1 = \mathbf{\hat{i}}$, $\mathbf{e}_2 = \mathbf{\hat{j}}$,
and $\mathbf{e}_3 = \mathbf{\hat{k}}$.


\bigskip
\noindent
In general,  the standard basis (which is sometimes called the computational 
basis)  for the $n$-dimensional Euclidean space consists of the ordered set
of $n$ distinct vectors in $\mathbb{R}^n$ 
...
\bigskip
\noindent
where $\mathbf{e}_i$ is the $i^\text{th}$ basis vector, that is, it
has a one in the $i^\text{th}$ coordinate (position) and zeros 
everywhere else.\footnote{This is also called a "one-hot" encoding 
in machine learning parlance, where the $i$'s might be the 
classes in a classification problem.} In matrix notation the 
$\mathbf{e}_i$ are the $n \times 1$ column vectors 
...
\bigskip
\noindent
Using these definitions we can define
the parametric form of some curve $C$ in 
$\mathbb{R}^3$, $\mathbf{r}(t)$, as follows:
...
\bigskip
\noindent
where $t \in [a,b]$ and $g(t)$, $h(t)$, and $k(t)$ are 
scalar functions\footnote{A scalar function is a function
$f$ such that $f: \mathbb{R}^n \to \mathbb{R}$.}.
...
generalization of the notion of length to vectors in $\mathbb{R}^n$.
...
\bigskip
\noindent
Then one way to prove the Cauchy–Schwarz inequality is to use the 
...
\bigskip
\noindent
Using the reverse triangle inequality we can see that
...
&\Rightarrow& 0 \leq \|\mathbf{x} - \mathbf{y}\|
					...
					= 1 \text{ by definition above, $1 - 1 = 0$, $|0| = 0$} \\
[8pt]
...
&\Rightarrow& 0 \leq \| \mathbf{x} \|^2 - 2 \langle \mathbf{x},\mathbf{y} \rangle 
					+ \|\mathbf{y}\|^2
					&\quad \mathrel{\#} {\| \mathbf{v} \|}^2 = \sum\limits_{j = 1}^{n} v_{j}^2
					\text{ and }
...
\bigskip
\noindent
If we replace $\mathbf{a}$ by $-\mathbf{a}$ 
we see by Equation (\ref{eqn:magnitude}) that 
$\|-\mathbf{a}\| = \| \mathbf{a}\|$. Further, by 
the scalar multiplication rule for dot products with $c_1 = -1$ 
and $c_2 
= 1$ we have $\langle -\mathbf{a},\mathbf{b} \rangle 
= - \langle \mathbf{a},\mathbf{b} \rangle$. Taken together
these results imply that\footnote{There is nothing special about using $\mathbf{a}$ 
...
\setstretch{1.10}
\noindent
So one way to see the Cauchy–Schwarz inequality 
...
$\big | \langle \mathbf{a},\mathbf{b} \rangle \big | \leq \|\mathbf{a}\| \|\mathbf{b} \|$,
namely, the Cauchy–Schwarz inequality. $\square$
