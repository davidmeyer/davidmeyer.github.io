...
definitions the parametric form of some curve $C$ in 
...
\bigskip
\noindent
where $t \in [a,b]$, $g(t)$, $h(t)$ and $k(t)$ are scalar 
functions, and $\mathbf{\hat{i}}$, $\mathbf{\hat{j}}$ and  
$\mathbf{\hat{k}}$ are the basis vectors \cite{notes:basis} 
for $\mathbb{R}^3$. $\mathbf{\hat{i}}$, $\mathbf{\hat{j}}$ and  
$\mathbf{\hat{k}}$ have column vector format 
