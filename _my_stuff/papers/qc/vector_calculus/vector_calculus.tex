...
\bigskip
\noindent
If $\mathbf{a}$ and $\mathbf{b}$ are column vectors then the dot
product can also be written as a matrix product: $\mathbf{a}
\cdot \mathbf{b} = \mathbf{a}^{\intercal}\mathbf{b}$ where
$\mathbf{a}^{\intercal}$ is the transpose of the vector
$\mathbf{a}$. Similarly, if $\mathbf{a}$ and $\mathbf{b}$ are row
vectors then the dot product can be written as $\mathbf{a} \cdot
\mathbf{b} = \mathbf{a}\mathbf{b}^{\intercal}$.
...
\rightarrow \underbrace{\begin{bmatrix} \hdots \hdots
\hdots \end{bmatrix}}_{1 \times n} \underbrace{\begin{bmatrix}
\hdots \\ \vdots \\ \hdots \end{bmatrix}}_{n \times 1}
\rightarrow c
\end{flalign*}
...
\bigskip
\noindent
for $p \geq 1, p \in \mathbb{R}$. The $p$-norm (also called ${\displaystyle \ell _{p}}$-norm) 
$\left\|\mathbf {x} \right\|_{p}$ is defined to be
...
then the cosine rule gives $\| \mathbf{x} - \mathbf{y} \|^2 
...
\bigskip
\begin{figure}[H]
\center{\includegraphics[frame,scale=0.30] {images/delta-r.png}}
