...
$\| \mathbf{x} \| = \|\mathbf{y} \| = 1$ ($\mathbf{x}$ and
$\mathbf{y}$ are unit vectors). Then we can see by the triangle inequality 
\cite{vector_norms_and_matrix_norms,wiki:triangle_inequality} that 

\begin{equation*}
\begin{array}{lllll}
\big |\|\mathbf{x}\| - \|\mathbf{y}\| \big |
&\leq& \|\mathbf{x} - \mathbf{y} \| 
					&\quad \mathrel{\#} \text{triangle inequality 
...
[8pt]
&\Rightarrow& 0 \leq \sum\limits_{j = 1}^{n} (x_j - y_j)^2
					&\quad \mathrel{\#} \text{by Equation (\ref{eqn:magnitude})} \\
[8pt]
&\Rightarrow& 0 \leq \sum\limits_{j = 1}^{n} \left ( x_{j}^2 - 2 x_{j} y_{j} + y_{j}^{2} \right )
					&\quad \mathrel{\#} \text{expand previous line} \\
[8pt]
&\Rightarrow& 0 \leq \sum\limits_{j = 1}^{n} x_{j}^2 - 2 \sum\limits_{j = 1}^{n} x_{j} y_{j} 
...
&\Rightarrow& 1 \geq \langle \mathbf{x},\mathbf{y} \rangle 
					&\quad \mathrel{\#} \text{divide by -2, flipping inequality}  \\
[12pt]
...
\bigskip
\noindent
If we replace $\mathbf{a}\footnote{There is nothing special about using 
$\mathbf{a}$ here. We could use $\mathbf{b}$ and get the same result.}$ by 
$-\mathbf{a}$ and since by Equation (\ref{eqn:magnitude}) we have 
$\|-\mathbf{a}\| = \| \mathbf{a}\|$ and by the scalar multiplication rule for dot products 
with $c_1 = -1$ and $c_2 = 1$ we have
$\langle -\mathbf{a},\mathbf{b} \rangle = - \langle \mathbf{a},\mathbf{b} \rangle$
we can see that
...
\bigskip
\noindent
For example, the wind vector field for the 2011 
...
&\Rightarrow& \Delta s \approx \bigg \| \dfrac{\mathbf{r}(t + \Delta t) - 
			...
			&\qquad \mathrel{\#} \text{this is our approximation} \\
[15 pt]
