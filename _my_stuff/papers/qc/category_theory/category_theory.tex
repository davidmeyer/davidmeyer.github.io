...
\bigskip
\noindent
\begin{notation} When $f$ is a monomorphism it is
often denoted with $\hookrightarrow$. So for example the
injection $i: \mathbb{Q} \to \mathbb{R}$ is frequently written as
...
\medskip
\noindent
is a natural transformation between the two functors (that is, 
$- \times S$ and $- \times T$). To show that $\text{id}\_ \times g$ 
is a natural transformation, pick two objects $A,B$ and a morphism 
$f : A \to B$. Then we need to show that the following diagram
commutes:
