...
\begin{center}																					% try to center the tikzpicture
   ...
   \begin{tikzpicture}
      \matrix[matrix of math nodes,																% use a TikZ matrix for the vertices
              ...
              \draw[->] (FA) edge node [midway,above,scale=\scale] {$F(f)$} (FB)				% F(A) -> F(B)
                        (GA) edge node [midway,below,scale=\scale] {$G(f)$} (GB); 				% G(A) -> G(B)
              \draw[->] (FA) edge node [midway,left,scale=\scale]  {$\eta_{A}$} (GA)			% F(A) -> G(A)
                        (FB) edge node [midway,right,scale=\scale] {$\eta_{B}$} (GB);			% F(B) -> G(B)
   \end{tikzpicture}
\end{center}
