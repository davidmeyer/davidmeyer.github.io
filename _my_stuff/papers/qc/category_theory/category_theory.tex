...
A category is a collection of objects together with 
a collection of composable morphisms between these objects.  
Note here that we say "class of objects" to sidestep the following
set-theoretic issue: if we instead used "the set of objects" then
in the category of sets the set of objects would we be the set of
all sets, which we know by Russell's Paradox does not exist
\cite{russells_paradox,zf_set_theory}. 
...
themselves. If the collection of objects is a set the category
is called a "small" category.

\noindent
The collection of objects in a category $\mathcal{C}$ is denoted 
simply as $\mathcal{C}$, or sometimes $\operatorname{Ob}(\mathcal{C})$.
Note that we refer to a generic category as $\mathcal{C}$, while bold
font is used to indicate a specific category (for example, the category 
of sets is called {\bf Set}). 

\item {\bf Morphisms (Arrows)}: Between any two objects in a
category, there exists a set of morphisms (also called arrows).
Morphisms can represent various mathematical structures like
functions, homomorphisms, transformations, etc. 
...
\medskip
\noindent
If the collection of morphisms in a category is a set then the 
category is called "locally small".
...
followed by $g$. Said another way: a category $\mathcal{C}$
...
an inverse.  Hence the morphisms in $\mathcal{C}(A,A)$ are not
required to be isomorphisms for $\mathcal{C}$ to be a monoid.
...
is called a monomorphism if for all $g_{1},g_{2} \in 
