...
\begin{definition} {\bf Monomorphism}: A morphism $g:B \to C$ is
called a \emph{monomorphism} if, for all $f_{i}: A \to B$ we have  

\begin{equation}
gf_{1} = gf_{2} \Rightarrow f_{1} = f_{2} 
...
(and not elements), since $g$ is a monomorphism both $f_{1}$ and
$f_{2}$ must map to map elements in $A$ to the same element in
$B$. This suggests that monomorphism is at least in part about
retaining uniqueness (or non-uniqueness) in the domain in the
codomain.  We can see this since we know by Equation
