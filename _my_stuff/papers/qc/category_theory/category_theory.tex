...
object, and call that object $A$. Since $\mathcal{C}$ only has
one object ($A$), all of $\mathcal{C}$'s morphisms are in
$\mathcal{C}(A,A)$.
...
\begin{itemize}
  \item the class of objects $\operatorname{\bf Obj}(\mathcal{C})$ consisting of the single object $A$ 
  \item the class of morphisms $\operatorname{\bf Hom}_{\mathcal{C}}(A,A) = \mathcal{C}(A,A)$ consisting of isomorphisms $f: A \to A$
...
$\mathcal{C}(A,A)$ into a group except for inverses. However, to
say that every morphism in $\mathcal{C}$ is an isomorphism is exactly
to say that every element of $\mathcal{C}(A,A)$ has an inverse
