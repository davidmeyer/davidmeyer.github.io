...
sine and then use its logarithmic form \cite{inverse_hyperbolic_sine}:

\bigskip
\begin{equation}
\int \dfrac{1}{\sqrt{x^{2} + 1}} \; dx = \sinh^{-1} x = 
...
\bigskip
\noindent
This holds for $c$ an integer, prime, real, or complex 
[@paulmasson@mathstodon.xyz].

\bigskip
\noindent
Now, if we take $x = \frac{1}{2}$ (Equation (\ref{eqn:lnphi})) 
...
\dfrac{c^2 -1}{2c}
&=& \dfrac{1}{2}			
				&\hspace{1em} \mathrel{\#} \text{set $x = \frac{c^2 -1}{2c} = \frac{1}{2}$} \\
[8pt]
...
&\Rightarrow& c^2 -c -1 = 0		
				&\hspace{1em} \mathrel{\#} \text{note that $x^2-x-1$ is the golden ratio's minimal polynomial \cite{wiki:golden_ratio}} \\
[8pt]
&\Rightarrow& c = \dfrac{-(-1) \pm \sqrt{(-1)^2 - 4 (1) (-1)}}{2 \cdot 1}	
				&\hspace{1em} \mathrel{\#} \text{solve for $c$ using the quadric formula \cite{wiki:quadradic_formula} } \\
[8pt]
&\Rightarrow& c = \dfrac{1 \pm \sqrt{5}}{2}	
				&\hspace{1em} \mathrel{\#} \text{simplify} \\
[8pt]
&\Rightarrow& c = \dfrac{1 + \sqrt{5}}{2}	
				&\hspace{1em} \mathrel{\#} \text{since we're taking the log, use the positive root} \\
[12pt]
&\Rightarrow& c = \phi	
				&\hspace{1em} \mathrel{\#} \text{definition of $\phi$} \\
...
\bigskip
\noindent 
@deilann@tech.lgbt notes that $c^2-c-1$ is (or at least should be :-)) immediately 
