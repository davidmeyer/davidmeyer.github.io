...
\bigskip
\noindent
Now we want to ask the question "what happens as $n$ goes to $\infty$"? In other words,
does this sequence converge, and if it does, to what value? The first step
is to take the limit of both sides:
...
\bigskip
\noindent
We do have some machinery we can use to evaluate this limit, including algebraic rules for limits \cite{properties_of_limits} 
and the fact that the limit is a linear operator \cite{fa_mit}. 

\bigskip
\noindent
Putting all of this together we can see that
...
[20pt]
&=& \dfrac{0 + 1} {0 + 1}
			&\qquad \qquad \qquad \mathrel{\#} {\displaystyle \lim_{n \to \infty} \frac{a}{n} = a \lim_{n \to \infty} \frac{1}{n} = 0} \text{ for constant $a$ \cite{limits_of_sequences}} \\
[10pt]
