...
\ref{section:expected_value_of_a_2_dollar_ticket} describes the
...
\times 10^6$. Then the expected number of winners is
...
\subsection{What is the Expected Value (jackpot only) of a Mega Millions Ticket?}
The expected jackpot payout per ticket, $\text{EV}_{\text{jackpot}}$, is:
...
\bigskip
\noindent
Substuting the values for Post-Tax Lump Sum Share and the Odds of Winning we get
...
\bigskip
\noindent
So given our assumptions the expected value of a Mega Millions lottery ticket is \$0.80.
...
that the net expected value of a \$2 ticket,
