...
Equation \ref{eqn:f(x)} since its an easier form to work with. In
addition, we can assume $a_{n-1} = 1$ since $f(x)$ is monic.}
...
\begin{array}{rlll} 
r^k 
&=& r^l                                                 &\qquad \qquad \mathrel{\#}  \text{$R$ is a finite integral domain} \\
[5pt]
&\Rightarrow& r \cdot r^{k-1} = r \cdot r^{l -1}        &\qquad \qquad \mathrel{\#}  \text{factor out $r$} \\
[5pt]
&\Rightarrow& r^{k-1} = r^{l -1}                        &\qquad \qquad \mathrel{\#}  \text{use cancellation law (cancel $r$, Equation \ref{eqn:cancellation_law}}) \\
[5pt]
&\Rightarrow& r \cdot r^{k-2} = r \cdot r^{l -2}        &\qquad \qquad \mathrel{\#}  \text{factor out $r$} \\
[5pt]
&\Rightarrow& r^{k-2} = r^{l -2}                        &\qquad \qquad \mathrel{\#}  \text{use cancellation law (cancel $r$, Equation \ref{eqn:cancellation_law})} \\
&\;\vdots												&&\qquad \qquad \mathrel{\#} \text{iterate $l - 1$ times} \\ 
[5pt]
&\Rightarrow& r^{k - l + 1} = r^1                       &\qquad \qquad \mathrel{\#}  \text{...} \\
[5pt]       
&\Rightarrow& r \cdot r^{k - l} = r \cdot r^0           &\qquad \qquad \mathrel{\#}  \text{factor out $r$} \\ 
[5pt]
&\Rightarrow& r^{k-l} = r^{0}                           &\qquad \qquad \mathrel{\#}  \text{use cancellation law (cancel $r$, Equation \ref{eqn:cancellation_law})} \\ 
[5pt]
&\Rightarrow& r^{k-l} = 1                               &\qquad \qquad\mathrel{\#} r^0 = 1
\end{array}
\end{equation*}
...
\in R$ is a unit and so $R$ is a field. $\square$
...
4)$ or $m^2 \equiv 1 \; (\mymod 4)$. $\square$ 
...
\begin{array}{rlll} 
r
&=& 1 \cdot r           &\qquad\qquad\mathrel{\#} \text{$1$ isthe multiplicative identity} \\
