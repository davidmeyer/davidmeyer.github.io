...
in the late $18^{\text{th}}$ century has a Roud Folk Song Index
number of 68 \cite{wikipedia:roud_folk_song_index} (if you happen
to be interested in that kind of thing). A large number of different 
melodies have been associated with the song, of which the best 
known is derived from a 1909 arrangement of a traditional folk melody 
by English composer Frederic Austin \cite{wikipedia:twelve_days_of_christmas}. 
There are twelve verses, each describing a gift given by "my true 
love" on one of the twelve days of Christmas. There are many variations 
in the lyrics, however, the lyrics given here are from Frederic Austin's
...
\medskip
\renewcommand*{\arraystretch}{1.90}					% get some extra space in the table
...
look at the {\bf Total number of gifts received by day n} column of
Table \ref{tab:12_days_of_christmas} we can see a pattern. In 
particular, the number of gifts received by (including) day 
{\bf n} is the sum of the gifts received on the previous days 
plus the gift received on day {\bf n}.  The number of gifts 
received by day {\bf n} is shown in red in Table \ref{tab:12_days_of_christmas}. 
These numbers are called the tetrahedral numbers and are defined as
follows:
...
Well, if so there would have to exist $m,n \in \mathbb{Z}^{+}$  
such that
...
\ref{fig:pascals_triangle}, contains the numbers we see in {\bf
Gifts received on day n} column of Table
...
\begin{figure}[H]
  ...
  \fbox {                                                                       % put a box around the figure
    \resizebox{0.85 \textwidth}{!} {                                            % resize the figure if you want
    \begin{mplibcode}                                                           %
...
triangular numbers and the tetrahedral numbers. So first, what
