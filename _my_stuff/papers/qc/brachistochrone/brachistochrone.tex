...
$ds^2 = dx^2 + dy^2$. This is shown in Figure \ref{fig:pythagorean_theorem}.
...
\centering
  ...
  \begin{tikzpicture} [every edge quotes/.append style = {anchor=south, sloped}, declare function={f(\x)=1 + \x*\x*\x;}]	% define f(x)
       \draw[thick, -latex] (-2,0) -- (5,0) coordinate [label={[xshift=0.30cm, below] $x$}];  								% x axis
       \draw[thick, -latex] (0,-1) -- (0,5) coordinate [label={[xshift=-0.30cm, left] $y$}]; 								% y axis
...
%	   
       ...
       \draw[red,thick] (5.65,4.5) circle (3.10);																		% big circle
%
%	draw the angle      
%       
...
\begin{figure}[H]
\center{\includegraphics[scale=0.55] {images/cycloid1.png}}
\caption{The Cycloid Curve}
\label{fig:cycloid1}
\end{figure}
...
the hanging chain problem, considers the shape of a stationary
