...
\bigskip
\noindent
BTW, how are the cycloid's coordinates computed? Consider the
scenario depicted in Figure \ref{fig:cycloid_coordinates}. To
find the coordinates of the point D we look at the distances 
involved. From the figure we can see that:

\medskip
\begin{itemize}
 \item The x-coordinate is the length of OT minus the length of DB.
 \item The y-coordinate is the length of CT minus the length of CB (note that CT = $a$).
...
the length of arc DT and arc DT is equal to a$\theta$ then
$a\theta = \text{arc DT} = \text{OT}$. Also, we know from
trigometry that $\text{DB} = a \sin \theta \text{ and } \text{CB}
= a \cos \theta$.  
...
\begin{itemize}
\item x-coordinate:  $\text{OT} – \text{DB} = a\theta – a \sin
\theta = a (\theta – \sin \theta)$ 
\item y-coordinate: $\text{CT} – \text{CB} = a – a \cos \theta =
a(1 – \cos \theta)$ 
\end{itemize}
...
\bigskip
\begin{figure}[H]
\center{\includegraphics[scale=0.65] {images/cycloid_coordinates.png}}
\caption{Computing Cycloid Coordinates \cite{cycloid_coordinates}} 
...
\bigskip
\noindent
Back to the history. So almost thirty years later, during the infancy of the
