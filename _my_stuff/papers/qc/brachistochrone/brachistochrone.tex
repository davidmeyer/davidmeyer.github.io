...
Here we are trying to find the \emph{path} that takes the shortest time from point 
$A$ to $B$. In the language of the calculus of variations \cite{wiki:cov} we want
...
\bigskip
\noindent
Consider a curve over in which we call an infinitesimal segment of the path is $ds$. Then
...
\bigskip
\noindent
Now, treating the derivative as a fraction\footnote{Care needs to be taken when 
...
\begin{equation*}
I = \int_{0}^{t} dt = \int_A^B \dfrac{ds}{V}
\end{equation*}

\bigskip
\noindent
Next, what can se say about $ds$? Well, we know from Pythagoras that 
$ds^2 = dx^2 + dy^2$ which is shown in Figure \ref{fig:pythagorean_theorem}.
...
[12pt]
&=& \left ( 1 + (y^{\prime})^2 \right ) dx^2					&\qquad \qquad \qquad \mathrel{\#} y^{\prime} = \dfrac{dy}{dx} \\
[10pt]
&\Rightarrow& ds = \sqrt{1 + (y^{\prime})^2} dx					&\qquad \qquad \qquad \mathrel{\#} \text{note: here we're treating $\dfrac{dy}{dx}$ as a fraction}
\end{array}
\end{equation*}
...
\begin{equation*}
\dfrac{1}{2} mV^2 - mgy = 0 \Rightarrow \dfrac{1}{2} mV^2 = mgy \Rightarrow 
\dfrac{1}{2} V^2 = gy \Rightarrow V = \sqrt{2gy}
\end{equation*}
...
&=& {\displaystyle \mathlarger{\mathlarger{\int_A^B}} \dfrac{ds}{V}}
									&\qquad \qquad \mathrel{\#} \text{definition of $I$} \\
[15pt]
...
\bigskip
\noindent
So we have this expression for $I$

\begin{equation*}
I = \dfrac{1}{\sqrt{2g}} \mathlarger{\mathlarger{\int_A^B}}  \sqrt{\dfrac{1 + (y^{\prime})^2}{y}} \; dx
\end{equation*}

\bigskip
\noindent
Recall that the Calculus of Variations is about finding a function that minimizes some other function. That 
"other function", which takes a function as an argument, is called a functional. Here $I$ is the functional 
the function that is trying to be minimized, $F$, is

\medskip
\begin{equation*}
F = \sqrt{\dfrac{1 + (y^{\prime})^2}{y}} \; dx
\end{equation*}

\bigskip
\noindent
The Beltrami Identity \cite{wiki:beltrami_identity} ...

\medskip
\begin{equation*}
F - y^{\prime} \dfrac{\partial F}{\partial y^{\prime}} = C
\end{equation*}
