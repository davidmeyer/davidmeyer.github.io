...
In Figure \ref{fig:pythagorean_theorem} we saw that we can solve
for an infinitesimal distance $ds$ using the Pythagorean Theorem: 
...
This is the hanging chain problem. Here $\rho$ is the density of
the chain, $g$ is the acceleration of gravity, and $A$ is the cross 
sectional area of the chain (which is assumed to be constant). Note that 
$\rho A = m$ since the units of $\rho$ are $\frac{\text{mass}}{\text{volume}}$ 
and since ${\displaystyle \lim_{ds \to 0} \text{Volume} = \text{Area}}$ 
