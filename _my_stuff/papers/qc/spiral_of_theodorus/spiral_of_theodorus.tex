...
fantastic object shown in Figure
\ref{fig:the_spiral_of_theodorus}.  It is composed of right
...
\bigskip
\noindent
The spiral is named after Theodorus of Cyrene, the ancient Greek
mathematician who lived during the $5^{\text{th}}$ century BC and
who apparently discovered the spiral
\cite{wiki:theodorus_of_cyrene}. Theodorus used the spiral to
prove that all of the non-square integers from 3 to 17 are
irrational. Theodorus' original spiral stops at $\sqrt{17}$,
apparently because $\sqrt{17}$ is the hypotenuse of the last
triangle in the spiral that doesn't overlap
\cite{wiki:spiral_of_theodorus}.

\bigskip
\noindent
Interestingly, in 1958 E. Teuffel showed that no two hypotenuses
of the spiral of Theodorus will ever coincide, regardless of how
far the spiral is continued. Apparently this was proven in "Eine
Eigenschaft der Quadratwurzelschnecke", Math.-Phys. Semesterber.
6 (1958), pp. 148-152 (does anyone have access to a copy of this
paper?).
...
\newcommand*{\sqrtspiral}[2][scale=3]{
    \begin{tikzpicture}[#1]														% start the tikzpicture
        ...
        \draw[\currentcolor] (A) edge node[fill=white]{$\sqrt{1}$}(B);			% draw first hypotenuse (of length \sqrt{1})
        \foreach \n in {1,...,\sqrtlast} {                                      % draw \n triangles
            \ifthenelse{\equal{\n}{\sqrtlast}}                                  % if it's the last one make it red
            {                                                                   %
...
%
\bigskip
\def \numberoftriangles {17}													% how many triangles (plus 1)
