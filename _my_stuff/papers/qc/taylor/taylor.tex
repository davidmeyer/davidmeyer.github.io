...
\begin{array}{lllll}
f(x)
&=& \sum\limits _{k=0}^{\infty}{\frac {f^{(k)}(a)}{k!}}(x-a)^{k}                                                                                                                    &\mathrel{\#} \text{Taylor series estimate of $f(x)$}                      \\  
[12pt]
...
\bigskip
\noindent
{\setstretch{1.95}The last line above, $R_n(x) = \frac{f^{(n+1)}(a)}{(n+1)!} (x-a)^{(n+1)}$, follows because the first $n$ terms of $f^{(n+1)}(x)= 0$ (they turn 
into constants whose derivative is zero) and $f^{(k)}(a) = 0$ for $k > n + 1$ since for $k > n + 1$ $f^{(k)}(a)$ has an $(x-a)^k$ term which is zero at $x = a$.
}
