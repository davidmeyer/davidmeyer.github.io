...
\noindent
\textbf{Proof.} Consider the case in which $R$ is an integral domain
(has no zero divisors) with $a \in R$ and $a \neq 0$. Then
...
[5pt]
&\Rightarrow& b-c = 0
			&\hspace{1em} \mathrel{\#} \text{$R$ has no zero divisors and $a \neq 0$ so $b-c$ must equal zero} \\
[5pt]
