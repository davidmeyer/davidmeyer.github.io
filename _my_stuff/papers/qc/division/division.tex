...
\smallskip
\begin{lemma}
$R$ has a cancellation law if and only if $R$ has no zero divisors.
\label{lemma:zero_divisors}
\end{lemma}


\noindent
\textbf{Proof.} Consider the case in which $a \in R$ and $a \neq 0$.
Then $a^{-1} \in R$ and
...
ab
&=& ac                                                  
			&\hspace{1em} \mathrel{\#} \text{by assumption} \\
[5pt]
&\Rightarrow& ab-ac = 0
			&\hspace{1em} \mathrel{\#} \text{subtract $ac$ from both sides} \\
[5pt]
&\Rightarrow& a\cdot(b-c) = 0
			&\hspace{1em} \mathrel{\#} \text{factor out $a$} \\
[5pt]
&\Rightarrow& a^{-1} \cdot \left (a\cdot(b-c) \right ) = 0
			&\hspace{1em} \mathrel{\#} \text{multiply both sides by $a^{-1}$} \\
[5pt]
&\Rightarrow& (a^{-1} \cdot a) \cdot (b-c) = 0
			&\hspace{1em} \mathrel{\#} \text{multiplication is associative} \\
[5pt]
&\Rightarrow& b-c = 0
			&\hspace{1em} \mathrel{\#} a^{-1} \cdot a = 1 \\
[5pt]
&\Rightarrow& b = c \hspace{1em} \blacksquare
\end{array}
\end{equation*}
...
r^k 
&=& r^l                                                 
			&\qquad \mathrel{\#} \text{$R$ is a finite integral domain} \\
[5pt]
&\Rightarrow& r \cdot r^{k-1} = r \cdot r^{l -1}        
			&\qquad \mathrel{\#} \text{factor out $r$} \\
[5pt]
&\Rightarrow& r^{k-1} = r^{l -1}                        
			&\qquad\mathrel{\#} \text{use cancellation law (cancel $r$, 
...
[5pt]
&\vdots                                                 
			&&\qquad\mathrel{\#} \text{iterate $l - 1$ times} \\ 
[5pt]   
...
and so has no zero divisors, see Theorem
\ref{thm:integral_domain}), it follows that the characteristic of
...
0$, we get a contradiction of Equation \ref{eqn:qminusqprime}. In
particular, by our convention the only way for this to happen is
for $\deg(q^\prime - q) = \deg(r^\prime - r) = - \infty$, and the
only way for that to happen is if $q^\prime - q = 0 = r^\prime -
