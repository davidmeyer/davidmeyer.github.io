...
\begin{figure}
\centering
  \resizebox{0.50 \textwidth}{!} {                                                                                                                                            % resize figure if you want (0.50 seems to be the same size)
    \begin{tikzpicture}  [every edge quotes/.append style = {anchor=south, sloped}]
       ...
       \draw[thick, ->] (0,-0.35) -- (0,5.5) 
            node[left] {$\mathbb{I}$}
            node[draw, very thin,below right=5mm] {$z = r \cdot \mathrm{e}^{i\theta} = r \cdot [\cos \theta + i \sin \theta]$};
...
\begin{array}{lllll}
e^{i\theta}
&=&                   \cos \theta + i \sin \theta              &\quad  \mathrel{\#} \text{Euler's Formula}                            \\
&\Rightarrow&  e^{i\pi} = \cos \pi + i \sin \pi          &\quad  \mathrel{\#} \text{set $\theta = \pi$}                          \\
...
\end{array}
\end{equation*}
\caption{Euler's Identity (well, its really Cotes' but ...)}
\end{figure}
