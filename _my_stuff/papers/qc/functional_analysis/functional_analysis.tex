...
topology $\tau_d$ on $X$, where $\tau_d :=
\{B_{\epsilon}(\mathbf{x}) \mid \mathbf{x} \in X \}$.
$B_{\epsilon}(\mathbf{x})$ is the open epsilon ball centered
at $\mathbf{x}$ (see Figure \ref{fig:open_epsilon_ball}). 
The pair $(X,\tau_d)$ is called a topological space.
