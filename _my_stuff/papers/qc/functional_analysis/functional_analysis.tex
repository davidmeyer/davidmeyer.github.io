...
space}\footnote{That is, $X$ is a vector space and
$\langle\cdot,\cdot\rangle: X \times X \rightarrow \mathbb{R}$ is
an inner product on the vector space $X$.} then
...
\sqrt{\langle
\mathbf{x}-\mathbf{y},\mathbf{x}-\mathbf{y}\rangle}$.  In
addition, the metric $d(\mathbf{x},\mathbf{y})$ induces a
topology $\tau_d$ on $X$, namely, $\tau_d =
\{B_{\epsilon}(\mathbf{x}) \mid \mathbf{x} \in X \}$ (see Figure
\ref{fig:open_epsilon_ball}). The pair $(X,\tau_d)$ is called a
topological space.
