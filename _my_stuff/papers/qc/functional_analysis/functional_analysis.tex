...
\date{Last Update: \today \\
	 {\vspace{1.00mm} \small Initial Version: June 8, 2018}}
%
%
%
\begin{document}
...
$(V,\langle\cdot,\cdot\rangle)$ is an \emph{inner product
space}\footnote{That is, $V$ is a vector space and
$\langle\cdot,\cdot\rangle: V \times V \rightarrow \mathbb{R}$ is
an inner product on the vector space $V$.} then
$(V,\norm{\cdot})$ is a normed space with norm $\norm{\mathbf{x}}
\coloneqq \sqrt{\langle \mathbf{x},\mathbf{x}\rangle}$ and
consequently $(V,d)$ is a metric space with metric
$d(\mathbf{x},\mathbf{y}) = \norm{\mathbf{x}-\mathbf{y}}
= \sqrt{\langle \mathbf{x}-\mathbf{y},\mathbf{x}-\mathbf{y}\rangle}$.
In addition, the metric $d(\mathbf{x},\mathbf{y})$ induces a 
topology $\tau_d$ on $V$, namely, $\tau_d = \{B_{\epsilon}(\mathbf{x}) \mid \mathbf{x} \in V \}$
 (see
Figure \ref{fig:open_epsilon_ball}). The pair $(V,\tau_d)$ is called 
a topological space.
