...
$(V,\langle\cdot,\cdot\rangle)$ is an \emph{inner product space}\footnote{That is, 
$V$ is a vector space and $\langle\cdot,\cdot\rangle: V \times V \rightarrow \mathbb{R}$ is an inner product on the
...
to inner product we are adding structure to a set or vector space.
More specifically
