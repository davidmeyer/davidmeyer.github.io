...
\backslash \{0\}$ (where 1 is the constant function returning 1). 
Ok, but why?
 
\medskip
\noindent
Well, notice that $\langle \hat{\mathbf{u}},\hat{\mathbf{u}} 
\rangle = 1$ for all unit vectors $\hat{\mathbf{u}}$, so what 
about $\langle 1,1 \rangle$? The issue is that $\langle 1,1 
\rangle = 2$:
