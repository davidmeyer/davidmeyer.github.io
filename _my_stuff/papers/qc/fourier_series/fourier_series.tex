...
$c$ with itself. Consequentially, I will use 1 to represent 
$f(x) = 1$ and use context to disambiguate the function $f(x)$ 
from the scalar value $1$. For example, in Equation (\ref{eqn:1}), 
$\langle 1,1 \rangle = \langle f(x),f(x) \rangle$ (1 represents 
the constant function $f(x) = 1$), while the notation $1 \cdot 1$ 
represents the scalar multiplication of the scalar value 1 with 
itself.
...
[10pt]
     ...
     {\LARGE \bf Orthonormal Basis $\left (\mathbb{R}^3 \right )$} 
                                                & $\{(1,0,0),(0,1,0),(0,0,1)\}$ 
                                                & $\{1, \cos mt, \sin nt\}, \, m,n \in \mathbb{N}\backslash\{0\}$ \\
[10pt]
     \hline
...
$\{1,\sqrt{2} \cos mt,\sqrt{2} \sin nt\}, \, m,n \in \mathbb{N}
\backslash \{0\}$. Ok, but why? Well, notice that $\langle
\hat{\mathbf{u}},\hat{\mathbf{u}} \rangle = 1$ for all unit
vectors $\hat{\mathbf{u}}$. However, $\langle 1,1 \rangle = 2$, 
since
...
\noindent
Why does Equation (\ref{table:vector_spaces}) hold? Consider

\bigskip
\begin{math}
  \begin{array}{llll}
     \langle 1,1 \rangle
    &=& {\displaystyle \frac{1}{\pi}\int\limits_{-\pi}^{\pi} f(x) f(x) \, dt}
     	&\hspace{2em} \mathrel{\#} \text{since in $1 = f(x)$ and the 
										 definition of the inner product 
										 (Table \ref{table:vector_spaces})} \\
    &=& {\displaystyle \frac{1}{\pi}\int\limits_{-\pi}^{\pi} 1 \cdot 1 \, dt}
     	&\hspace{2em} \mathrel{\#} \text{since in $f(x) = 1$ for all 
										 $x \in \mathbb{R}$} \\
...
\bigskip
\noindent
 We can also see that the vectors in the basis $\{1, \cos mt,
 \sin nt\}, m,n \in \mathbb{N}\backslash\{0\}$ are 
 orthogonal:
 
\begin{equation*}
  \langle 1,\sin nt \rangle = {\displaystyle
...
\smallskip
\noindent
Similarly, $\langle 1,\cos nt \rangle = \langle \cos mt,\sin nt
\rangle = 0$.
