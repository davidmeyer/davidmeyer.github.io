...
Here we are using the convention of using the constant $c$ to
denote the constant function $f(x) = c$, for all $x$. So we'll
use 1 to represent $f(x) = 1$ and use context to disambiguate the
function $f(x)$ from the scalar value $1$. For example, in
Equation (\ref{eqn:1}) $\langle 1,1 \rangle = \langle f(x),f(x)
\rangle$ (1 represents the constant function $f(x) = 1$), while
the notation $1 \cdot 1$ represents the scalar multiplication of
the scalar value 1 with itself.
