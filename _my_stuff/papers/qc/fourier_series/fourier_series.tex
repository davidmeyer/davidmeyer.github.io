...
feel that the orthonormal basis for the Fourierseries should be 
$\{1,\sqrt{2} \cos mt,\sqrt{2} \sin
nt\}, \, m,n \in \mathbb{N}\backslash\{0\}$. Ok, but why?
 
\medskip
\noindent
First, notice that $\langle \hat{\mathbf{u}},\hat{\mathbf{u}} 
\rangle = 1$ for all unit vectors $\hat{\mathbf{u}}$, so what 
about $\langle 1,1 \rangle$, where 1 is the constant function 
returning 1? The issue is that $\langle 1,1 \rangle = 2$:

 
\bigskip
\begin{equation*}
  \langle 1,1 \rangle = 
     \frac{1}{\pi}\int\limits_{-\pi}^{\pi} 1^2 dt	= 
...
unit vectors $\hat{\mathbf{u}}$, so what about $\langle 1,1 \rangle$ 
if the constant function 1 is part of an orthonormal basis
(again, since $\langle 1,1 \rangle = 2$)?
...
\bigskip
\noindent
Similarly, $\langle 1,\cos nt \rangle = \langle \cos
mt,\sin nt \rangle = 0$. 
