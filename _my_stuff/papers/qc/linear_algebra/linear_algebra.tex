...
Now define $f_{\bm{A}}: \mathbb{R}^{n} \rightarrow \mathbb{R}^{m}$ by 
...
\item $f_{\bm{A}}(\mathbf{x}+\mathbf{y}) = f_{\bm{A}}(\mathbf{x})+f_{\bm{A}}(\mathbf{y}) \Rightarrow		
		...
		\hspace{0.70em} ($f_{\bm{A}}$ is distributive; definition \ref{def:linear_map} 
...
\item $f_{\bm{A}}(\lambda \mathbf{x}) = \lambda f_{\bm{A}}(\mathbf{x}) \Rightarrow				
		...
		\hspace{8.50em} ($f_{\bm{A}}$ is compatible; definition \ref{def:linear_map} 
...
\medskip
\noindent
So we see that $\bm{A}(\mathbf{x}+\mathbf{y}) = \bm{A}\mathbf{x}+ 
\bm{A}\mathbf{y}$ (this is definition \ref{def:linear_map} 
\ref{enumerate:distributive} above \cite{wiki:linear_map}).
\end{proposition}
