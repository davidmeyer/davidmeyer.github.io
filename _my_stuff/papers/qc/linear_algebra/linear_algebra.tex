...
\item $f(\lambda \mathbf{x}) = \lambda f(\mathbf{x})$				
		\hspace{7.25em} \# scalar multiplication is
...
associated with those groups. So a linear map is a homomorphism of vector spaces. However, 
...
and define $f_{\bm{A}}: \mathbb{R}^{n} \rightarrow \mathbb{R}^{m}$ such that 
...
\medskip
\begin{enumerate}[label=(\alph*)]
\item $f_{\bm{A}}(\mathbf{x}+\mathbf{y}) = f_{\bm{A}}(\mathbf{x})+f_{\bm{A}}(\mathbf{y}) \Rightarrow		
		\bm{A}(\mathbf{x} + \mathbf{y}) = \bm{A}\mathbf{x} + \bm{A}\mathbf{y}$
		\hspace{1.70em} \# $f_{\bm{A}}$ is distributive (definition \ref{def:linear_map} 
		\ref{enumerate:distributive} above)

\item $f_{\bm{A}}(\lambda \mathbf{x}) = \lambda f_{\bm{A}}(\mathbf{x}) \Rightarrow				
		\bm{A}(\lambda \mathbf{x}) = \lambda \bm{A}\mathbf{x}$
		\hspace{8.55em} \# $f_{\bm{A}}$ is compatible (definition \ref{def:linear_map} 
		\ref{enumerate:compatible} above)
\label{enumerate:compatible}
\end{enumerate}

\noindent
For example, let $\bm{A} \in \mathbb{R}^{m \times 2}$ with $\bm{A} = 
	\left (
		\hspace{-0.50em}
		\begin{array}{cccc}
			\vertbar & \vertbar \\
...
&=&
	...
	\Bigg (
		...
		\end{array}
		\hspace{-0.50em}
	\Bigg ) 												&\hspace{3.50em}\mathrel{\#} \text{matrix (vector) addition in $\mathbb{R}^{2}$} \\
[25pt]
&=&
	\left (
		\hspace{-0.50em}
...
\left (
	...
	\left (
		...
		\hspace{-0.50em}
	\right )
	y_2													&\hspace{3.50em}\mathrel{\#} \text{vector addition is commutative} \\
[25pt]
&=&
\left (
		\hspace{-0.50em}
...
\bigskip
\noindent
So $\bm{A}(\mathbf{x}+\mathbf{y}) = \bm{A}\mathbf{x}+\bm{A}\mathbf{y}$. Here 
we see that the matrix $\bm{A}$, which is just a table of real (or complex) 
numbers, induces the abstract linear map $f_{\bm{A}}$ (and vice versa) 
