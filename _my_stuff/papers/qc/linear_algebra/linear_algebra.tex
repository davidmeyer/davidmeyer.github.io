...
linear combination \cite{wiki:linear_subspace}.  That is, all 
linear combinations of $U$ are in $U$. More specifically, let 
$\mathbf{u}^{(1)},\mathbf{u}^{(2)},\hdots, \mathbf{u}^{(k)}$ be 
vectors in $U$ and let $\lambda_{1},\lambda_{2},\hdots, \lambda_{k}$ 
be scalars in $\mathbb{R}$. Then
...
A $U \subseteq \mathbb{R}^{n}$ is a (linear) subspace iff
...
\medskip
\noindent
As an example of a linear subspace, consider a line $U \subset
\mathbb{R}^{2}$.  Then for vectors $\mathbf{u}, \mathbf{v} \in 
U$ and $\lambda \in \mathbb{R}$ we have $\lambda \mathbf{u} 
\in U$ and $\mathbf{u}+\mathbf{v} \in U$. Thus $U$ is a linear 
subspace of $\mathbb{R}^{2}$. This is (hopefully) shown Figure 
\ref{fig:linear_subspace}.

\bigskip
\begin{figure}[H]
  \centering
  \resizebox{0.45 \textwidth}{!} {              % resize figure here if you want
    \begin{tikzpicture}[scale=0.5]              % can also scale here
       ...
       \coordinate (EP)         at (7,5);       % endpoint (U)
       \coordinate (SP)         at (-2.8,-2);   % start point (U)
       \coordinate (uEP)        at (1.4,1);     % end point (u)
       \coordinate (vEP)        at (2.8,2);     % end point (v)
       \coordinate (u+vEP)      at (4.2,3);     % end point (u+vEP)
%	
...
%
       ...
       \draw[thick,magenta] (SP) -- (EP) node [right,xshift=1.50mm,yshift=1.50mm] 
                        {\color{black}${\color{magenta} U} \subset \mathbb{R}^{2}$}; 
%
%	draw vectors u, v and u+v
%    
		\draw[thick,green,->](O) -- (uEP) node
                     [right,below,xshift=1.25mm,yshift=-0.50mm] 
                     {$\color{green}\mathbf{u}$}; 
		\draw[thick,red,->](uEP) -- (vEP) node
                     [right,xshift=0.25mm,yshift=-0.50mm]
                     {${\color{red}\mathbf{v}}
                       {\color{black}=} 
                       {\color{black} \lambda}
                       {\color{green}\mathbf{u}}$};  
...
\mathbb{R}^{n} \rightarrow \mathbb{R}^{m}$ such that $\mathbf{x}
