...
\bigskip
\noindent
Suppose $f$ is a linear map and $\bm{x} = 
\left (\begin{array}{cccc} x_1 \\ \vdots \\ x_n \end{array} \right)$. 
Further, define the canonical basis vectors for $\mathbb{R}^{n}$ as
follows:

\begin{definition}
\label{def:canonical_basis_vectors}
{\bf Canonical Basis Vectors: } The canonical basis vectors for 
$\mathbb{R}^{n}$ are to be the vectors $\mathbf {e} _{i}$ for
...
everywhere else\footnote{The canonical basis vectors look surprisingly 
like the one-hot encoding used in machine learning \cite{wiki:one-hot}.}. 
...
\bigskip
\noindent
Here's an interesting observation about linear maps:
