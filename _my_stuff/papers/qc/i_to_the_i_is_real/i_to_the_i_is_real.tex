...
%
\title{Why is $i^i$ a real number and what does it equal?}
...
\cite{notes:eulers_formula} that $i = e^{i \frac{\pi}{2}}$ 
(since $e^{i\frac{\pi}{2}} = \cos \frac{\pi}{2} + i \sin \frac{\pi}{2} = i$).  
Note that the same result can be obtained with $x \in \left \{\frac{\pi}{2},
...
\begin{itemize}
\item It would be helpful to have a proof that complex conjugation 
distributes over complex exponentiation. In particular, a short 
proof that $\overline{i^i} = {\overline{i}}^{\overline{i}}$
would improve Section \ref{sec:automorphism} of this note.

\smallskip

One argument is that since $i^i \in \mathbb{R}$ (as shown above) and the complex
conjugate of a real number is itself, it must be that $\overline{i^i} = i^i$. But AFAICU 
this doesn't show that $\overline{i^i} = {\overline{i}}^{\overline{i}}$.
\end{itemize}
%
%
%
