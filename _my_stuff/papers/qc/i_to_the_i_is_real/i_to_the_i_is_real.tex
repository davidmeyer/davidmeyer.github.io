...
usually corresponds to the choice $k=0$
\cite{wiki:principle_branch}.  One note: here I will use log to
refer to the the principal value, noting that some authors Log to
distinguish the principal value from other logarithms of $z$
\cite{wiki:complex_logarithm}. See Remark
\ref{remark:complex_logarithm} for a bit more on this point.
%
%
%
...
equals $r$. In particular, why does a complex number $z = a + bi
= r \mathrm{e}^{i \theta}$? To see why this is the case consider
the complex plane, shown in Figure \ref{fig:complex_plane}:

\medskip
\begin{figure}[H]
\centering
  \resizebox{0.375 \textwidth}{!} {														% resize figure if you want
...
\bigskip
\noindent
This is all good, but still, why does $\overline{i^i} = {\overline{i}}^{\overline{i}}$?
...
\bigskip
\noindent
So $\log (\overline{z}) = \log(r) - \log(e^{i\theta})$.

\bigskip
\noindent
The next result we need is $\log (\overline{z}) = \overline{\log(z)}$.
...
\bigskip
\noindent
Now we know that $\log (\overline{z}) = \overline{\log(z)}$ and so 

\bigskip
\begin{equation*}
\begin{array}{llll}
\log(\overline{z^w})
&=& \overline{\log(z^w)}                        &\hspace{9em} \mathrel{\#} \text{as shown above} \\
[8pt]
&=& \overline{w\log(z)}                         &\hspace{9em} \mathrel{\#} \text{by the power rule for logarithms} \\
[8pt]
&=& \overline{w} \cdot \overline{\log(z)}       &\hspace{9em} \mathrel{\#} \text{by the product rule for conjugation} \\
[8pt]
&=& \overline{w} \cdot \log(\overline{z})       &\hspace{9em} \mathrel{\#} \text{since $\log (\overline{z}) = \overline{\log(z)}$} \\
[8pt]
&=& \log(\overline{z}^{\overline{w}})           &\hspace{9em} \mathrel{\#} \text{by the power rule for logarithms}
\end{array}
\end{equation*}

\bigskip
\noindent
Since $e^{\log(\overline{z^w})} = e^{\log(\overline{z}^{\overline{w}})}
\Rightarrow \overline{z^w} = \overline{z}^{\overline{w}}$ if we set
$z=w=i$ we see that $\overline{i^i} = {\overline{i}}^{\overline{i}}$. 
$\blacksquare$
