...
There are a few ways to think about this question. But first we
look at why a complex number $z$ equals $r\mathrm{e}^{i \theta}$.
Then in Section \ref{sec:automorphism} we use the fact that
complex conjugation is an automorphism of $\mathbb{C}$ to show
that $i^i \in \mathbb{R}$.  In Section \ref{sec:power_series} we
use the power series expansion of $i^i$ to show that $i^i \in
\mathbb{R}$, and in Section \ref{sec:eulers_formula} we rely on
Euler's formula to show the same result. In Section
\ref{what_does_i_to_the_i_equal} we do a bit of arithmetic to
show the numerical value of $i^i$. Finally Section
\ref{sec:conclusions} offers a few conclusions.

\bigskip
\noindent
Before launching into all of this, note that we will make heavy
use of the exponential function $\exp(z)$ (and therefore as you
might expect, this means heavy use of $\log(z)$, where by
$\log(z)$ we mean $\log_{e}(z)$ aka $\ln (z)$). The important
...
re^{i\theta + 2\pi ik}$ for all $k \in \mathbb{Z}$ we have that
for any choice of $k$, $\log(re^{i\theta+2\pi ki})$ is a valid
inverse for $re^{i\theta}$. The logarithm is therefore a
multivalued function and each value of $k$ defines what is called
a different \emph{branch} of the logarithm. The principal branch
usually corresponds to the choice $k=0$ \cite{wiki:principle_branch}.
...
$a^c \times b^c$ only equals $(ab)^c$ when the sum of the principal 
arguments \cite{principal_argument} of $a$ and $b$ equals the principal 
argument of $ab$. Apparently a similar problem arises for conjugacy 
when $a$ is strictly negative.
\end{remark}
%
%
%
\section{First: Why does a complex number $z$ equal $r\mathrm{e}^{i \theta}$?}
...
$z = a + bi = r \mathrm{e}^{i \theta}$? Consider the complex plane, 
which is shown in Figure \ref{fig:complex_plane}:

\medskip
\begin{figure}[H]
\centering
  \resizebox{0.31 \textwidth}{!} {														% resize figure if you want
...
\smallskip
\noindent
Given Figures \ref{fig:complex_plane} and \ref{fig:eulers_formula} it is pretty easy to see 
why $z = r \mathrm{e}^{i\theta}$:
\begin{equation*}
\begin{array}{lllll}
z
&=& a + ib
...
\bigskip
\noindent
To see \ref{item:overline_z}., consider
\begin{equation*}
\begin{array}{llll}
\overline{z}
&=& \overline{r \cdot (\cos \theta + i \sin \theta)} 
		&\hspace{5em} \mathrel{\#} \text{since $z = r(\cos\theta+i\sin\theta)$ (Figure \ref{fig:complex_plane})}\\
[5pt]
&=& \overline{r} \cdot \overline{(\cos \theta + i \sin \theta)} 
		...
		(product rule for complex conjugates \cite{product_complex_conjugates})} \\
[5pt]
&=& r \cdot \overline{(\cos \theta + i \sin \theta)} 
		&\hspace{5em} \mathrel{\#} \text{since $\overline{r} = \overline{r + 0i} = r - 0i = r$ 
		($\overline{r} = r$ for $r \in \mathbb{R}$)} \\
[5pt]
&=& r \cdot (\cos \theta - i \sin \theta)
		&\hspace{5em} \mathrel{\#} \text{since $\overline{a+bi} = a-bi$} \\
[5pt]
&=& r \mathrm{e}^{-i\theta}
		&\hspace{5em} \mathrel{\#} \text{since $e^{-i\theta} = \cos \theta - i \sin \theta$ (corollary to 
		Euler's formula \cite{eulers_formula_corollary})}
\end{array}
\end{equation*}
...
show that $i^i \in \mathbb{R}$ by the above argument. So
consider:
...
[2pt]
&=& {(i^{-1})}^{-i}                     &\hspace{13em} \mathrel{\#} \text{since $-i = i^{-1}$ (see item \ref{item:minus_i}. above)} \\
[2pt]
...
\bigskip
\noindent
All ok, but why does $\overline{i^i} = {\overline{i}}^{\overline{i}}$?
...
\begin{array}{llll}
\log (\overline{z})
&=& \log(\overline{re^{i\theta}})
		&\hspace{7em} \mathrel{\#} \text{since $z = re^{i\theta}$ (Figure \ref{fig:complex_plane})} \\
[12pt]
&=& \log(r \mathrm{e}^{-i\theta})
		&\hspace{7em} \mathrel{\#} \text{by item \ref{item:overline_z}. above} \\
[8pt]
...
\begin{equation}
\log (\overline{z}) = \log(r) - \log(e^{i\theta})
\label{eqn:overline_z_equals}
\end{equation}
...
&=& \overline{\log(r) + \log(e^{i\theta})}
                &\hspace{7em} \mathrel{\#} \text{by the product rule for logarithms} \\
[5pt]
...
&=& \log(r) + \overline{\log(e^{i\theta})}
                &\hspace{7em} \mathrel{\#} \text{since $\log(r) \in\mathbb{R}$} \\
[5pt]
&=& \log(r) + \overline{i\theta}
                &\hspace{7em} \mathrel{\#} \text{since $\log (e^{i\theta}) = i\theta$} \\
[5pt]
&=& \log(r) - i\theta
                &\hspace{7em} \mathrel{\#} \text{since $\overline{i\theta} = \overline{0+i\theta} = -i\theta$} \\
[5pt]
&=& \log(r) - \log (e^{i\theta})
                &\hspace{7em} \mathrel{\#} \text{since $i \theta = \log (e^{i\theta})$} \\
[5pt]
&=& \log (\overline{z})
                &\hspace{7em} \mathrel{\#} \text{by Equation (\ref{eqn:overline_z_equals})} \\
\end{array}
\end{equation*}
...
\begin{array}{llll}
\log(\overline{z^w})
&=& \overline{\log(z^w)}                        &\hspace{10em} \mathrel{\#} \text{Equation (\ref{eqn:log_of_conjugate})} \\
[5pt]
&=& \overline{w\log(z)}                         &\hspace{10em} \mathrel{\#} \text{by the power rule for logarithms} \\
[5pt]
&=& \overline{w} \cdot \overline{\log(z)}       &\hspace{10em} \mathrel{\#} \text{by the product rule for conjugation} \\
[5pt]
&=& \overline{w} \cdot \log(\overline{z})		&\hspace{10em} \mathrel{\#} \text{by Equation (\ref{eqn:log_of_conjugate})} \\
[5pt]
&=& \log(\overline{z}^{\overline{w}})           &\hspace{10em} \mathrel{\#} \text{by the power rule for logarithms}
\end{array}
\end{equation*}
...
\begin{equation}
\log(\overline{z^w}) = \log(\overline{z}^{\overline{w}})
\label{eqn:overlines}
\end{equation}

\bigskip
\noindent
Exponentiating both sides of Equation (\ref{eqn:overlines}) gives us

\begin{equation}
\overline{z^w} = \overline{z}^{\overline{w}}
\label{overline_z_to_the_w}
\end{equation}

\smallskip
\noindent
So now if we set $z=w=i$ in Equation (\ref{overline_z_to_the_w})
we see that $\overline{i^i} = {\overline{i}}^{\overline{i}}$.
$\blacksquare$
...
\bigskip
\begin{figure}[H]
\center{\includegraphics[frame,scale=0.35] {images/e_to_the_minus_pi_over_two.png}}
\end{figure}
%
%
%
...
(@AndresCaicedo@mathstodon.xyz), Ben Reiniger 
(@bmreiniger@mathstodon.xyz) and Dave Neary (@dneary@mastodon.ie) for 
