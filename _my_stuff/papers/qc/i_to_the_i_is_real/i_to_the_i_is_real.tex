...
re^{i(\theta+2k\pi)}$ for all $k \in \mathbb{Z}$ we have that
for any choice of $k$, $\log(re^{i(\theta+2k\pi)})$ is a valid
...
One note: here I will use log to refer to the the principal value,
noting that some authors Log to distinguish the principal value 
from other logarithms of $z$ \cite{wiki:complex_logarithm}. See
Remark \ref{remark:complex_logarithm} for a bit more on this point.
%
%
%
\section{First: A Bit of Complex Number Review}
\label{sec:why_is_z_equal_r}
This section reviews the nature of a complex number $z$,
where the length of the line from the origin to the point $z$,
$|z|$, equals $r$. In particular, why does a complex number
$z = a + bi = r \mathrm{e}^{i \theta}$? To see why this is the
case, consider the complex plane, shown in Figure 
\ref{fig:complex_plane}:
...
\bigskip
\noindent
All good, but still, why does $\overline{i^i} = {\overline{i}}^{\overline{i}}$?
...
\bigskip
\noindent
The next result we need is that $\log (\overline{z}) = \overline{\log(z)}$,
since
...
\bigskip
\noindent
So now we know that $\log(\overline{z^w}) = \log(\overline{z}^{\overline{w}})$.
Exponentiating both sides of $\log(\overline{z^w}) = \log(\overline{z}^{\overline{w}})$ 
gives us $\overline{z^w} = \overline{z}^{\overline{w}}$ and setting $z=w=i$ we 
