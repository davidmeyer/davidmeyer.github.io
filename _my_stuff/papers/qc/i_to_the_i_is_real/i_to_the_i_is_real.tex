...
inverse for $re^{i\theta}$. The logarithm is 
therefore a multivalued function and each value of $k$ defines 
what is called a different \emph{branch} of the logarithm. The 
principal branch \cite{wiki:principle_branch} usually corresponds 
to the choice $k=0$.
%
%
%
...
&=& \overline{r (\cos \theta + i \sin \theta)} 
		&\hspace{5em} \mathrel{\#} \text{Euler's formula} \\
[5pt]
&=& \overline{r} \cdot \overline{(\cos \theta + i \sin \theta)} 
		&\hspace{5em} \mathrel{\#} \text{$\overline{z_1 \cdot z_2} = \overline{z_1}\cdot \overline{z_2}$ 
...
[5pt]
&=& r \mathrm{e}^{-i\theta}
		&\hspace{5em} \mathrel{\#} \text{Euler's formula, $\theta \in \mathbb{R}$, 
		  $\overline{e^{i\theta}} = e^{\overline{i\theta}}= e^{\overline{i}\theta} 
		  = e^{-i\theta}$, and $\overline{i} = -i$ (by \ref{item:minus_i}. above)}
\end{array}
\end{equation*}
...
\begin{array}{llll}
\overline{i^i}
&=& {\overline{i}}^{\overline{i}}       &\hspace{7em} \mathrel{\#} \text{see Section \ref{sec:conclusions}} \\
[2pt]
...
\begin{itemize}
\item It would be helpful to have a proof that complex
conjugation distributes over exponentiation. In particular, a
short proof that $\overline{i^i} = {\overline{i}}^{\overline{i}}$
would improve Section \ref{sec:automorphism} of this note.

\smallskip

One argument is that since $i^i \in \mathbb{R}$ (as shown above)
and the complex conjugate of a real number is itself, it must be
that $\overline{i^i} = i^i$. But AFAICU this doesn't show that
$\overline{i^i} = {\overline{i}}^{\overline{i}}$.
\end{itemize}
%
%
%
