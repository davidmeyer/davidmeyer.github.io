...
\cite{wiki:golden_ratio}. The progression means that the triangle
has sides $1:\sqrt{\phi}:\phi$. The triangle is named after the famous German 
...
{\setstretch{1.15}
\noindent
Note that if the common ratio of the geometric progression is
$\sqrt{x}$, then the sides of the triangle are in the ratio 
$1 : \sqrt{x} : x$. So we know by the Pythagorean theorem
\cite{wiki:trig} that $1^{2} +(\sqrt{x})^{2} = x^{2}$. Simplifying
we get $1 +x = x^{2}$, or in a more standard form $x^{2}-x-1 = 0$. 
This polynomial turns out to be the minimal characteristic polynomial 
of the golden ratio $\phi$, so $x = \phi$.
