...
\bigskip
\noindent
Hooke's Law describes a form of simple harmonic motion, which as
mentioned above occurs when the restoring force in a system (the
force directed toward a stable equilibrium point) is proportional
to the displacement from that equilibrium. Hooke's Law is most
frequently stated as\footnote{It seems to be convention to use $x$ 
and $x(t)$, $F$, $F(x)$ and $F(x(t))$, and $a$ and $a(t)$
interchangeably.} $F \propto -x$. That is, the force $F$ is
...
&\Rightarrow& a = - \dfrac{k}{m} x
        &\hspace{8em}\mathrel{\#} \text{solve for $a$, noting that $a$ and $x$ are functions of time} \\
[7pt]
...
\bigskip
\noindent
This setup is shown modeling a spring and mass system\footnote{Where the 
force is proportional to the distance $x$ that the spring is stretched from 
its equilibrium position.} in Figure \ref{fig:spring_and_mass_system}.
...
energy (PE) of such as system? Well, we know that $\text{KE} =
\frac{1}{2} mv^{2}$ (where $v := \norm{\mathbf{v}}$; see
\cite{dmm:vector_calculus,kinetic_energy} for more), but what
...
{\setstretch{1.10}
\noindent
The potential energy is defined to be the work done by the system
\cite{wiki:potential_energy}.  Why? It turns out that the work
done by a conservative force is equal to the area under the force
curve \cite{wiki:conservative_force}. Figure \ref{fig:PE} is a
graph of a linear force $F$ versus position $x$, where the 
slope of the line is $k$. In this case the area under the curve 
is $\frac{1}{2} kx^2$ and thus the work done or potential energy 
stored is $\frac{1}{2} kx^2$. See \cite{elastic_potential_energy}
for more.
\par}
...
\centering
  ...
  \begin{tikzpicture}
       ...
       \coordinate (A)					at (4.0,6.15);			% A = ... rectangle
%
%
%
...
%	Label the origin
%
       \node [xshift=-0.35cm,yshift=-0.35cm,scale=1.15] at (0,0) {$O$};
%
...
%
	   \node [draw,rectangle,blue,scale=1.20] at (A) 
	   		{$\color{black}{\text{A} = 
	   		  \int_{x} F(x) \, dx    =
			  \int_{x} kx \, dx      = 
			  \frac{1}{2}kx^{2}}$
...
&=& \dfrac{1}{2}mv^2 + \dfrac{1}{2}kx^2 									
						&\qquad \qquad \qquad \mathrel{\#} E_{\text{total}}
                                                = \text{KE} + \text{PE} \\
[12pt]
...
\bigskip
\noindent
So we see that the oscillator has $E_{\text{total}} = 2
...
mass and the small-angle pendulum systems,
...
ma 
&=& - kx -b \dot{x} 
		&\qquad \qquad \mathrel{\#} \text{Equation (\ref{eqn:balancing_force}) minus the damping force $b \dot{x}$} \\
[14pt]	
&=& -b \dot{x} - kx 
		&\qquad \qquad \mathrel{\#} \text{rearrange}	\\
[14pt]	
...
again the equation of motion, Equation
(\ref{eqn:damped_spring_and_mass}), is of the same form as
Equation (\ref{eqn:damp_general_form}).  Finally, the
displacement function for the damped harmonic oscillator is shown
...
particle will feel and oscillating force. The charge carried by a
...
suggestions on Figure \ref{fig:simple_pendulum}. Thanks also to
