...
\noindent
It turns out
that the potential energy at a position $x$ is equal to the area 
under the force function curve between the origin and $x$.
Figure \ref{fig:PE} attempts to explain why $\text{PE} = \frac{1}{2}kx^2$ 
for the force function $F(x) = -kx$ when only the magnitude of 
$F(x)$ is considered. That is, $F(x) = \norm{-kx} = kx$. 
