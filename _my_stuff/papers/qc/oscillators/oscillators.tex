...
vectors (aka scalars). So it seems to be convention to use, for 
example, $F = ma$ rather than $\mathbf{F} = m \mathbf{a}$, when
working in $\mathbb{R}^{1}$.
...
total energy $E(x(t),\dot{x}(t))$ is independent of $t$.
...
as it moves according to Newton’s Second Law. $\blacksquare$
\end{theorem}
