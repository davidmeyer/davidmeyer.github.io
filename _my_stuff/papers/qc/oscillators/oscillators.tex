...
$\dfrac{d}{dt}E(x(t),\dot{x}(t)) = 0$. Computing this derivative 
requires some machinery from the chain, power, and product 
rules \cite{wiki:chain_rule,wiki:power_rule}:
\par}
\smallskip
%
%
...
&=& \dot{x}(t)\Big [m \ddot{x}(t) - F(x(t)) \Big ]
		&\hspace{1em} \mathrel{\#} \text{since $\dfrac{d}{dx}V(x(t)) = - F(x(t))$ 
		                                 (Equation (\ref{eqn:F}))} \\
[15pt]
&=& \dot{x}(t) \cdot 0
		&\hspace{1em} \mathrel{\#} \text{by Newton's $2^{\text{nd}}$ Law: 
		                                 $F(x(t)) = m\ddot{x} \Rightarrow m 
		                                 \ddot{x}(t) - F(x(t)) = 0$} \\
[15pt]
&=& 0
		&\hspace{1em} \mathrel{\#} \dfrac{d}{dt} E(x(t),\dot{x}(t)) = 0 
\end{array}
\end{equation*}
