...
curve $F$ \cite{wiki:conservative_force}. Since potential energy is 
a scalar quantity \cite{elastic_potential_energy} we are only 
interested in the magnitude of $F$, $kx$, at each point $x$ (the sign 
might be taken as the the direction of the $F$ vector in the one-dimensional 
case). So if we set $F_{m} = kx$ we get the graph in Figure \ref{fig:PE}, 
where we can see that the area under the curve is $\frac{1}{2} kx^2$. 
Thus the work done or potential energy stored is $\frac{1}{2} kx^2$. 
\par}
%
