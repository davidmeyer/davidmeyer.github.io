...
%
      ...
      \draw [thick,brown,-stealth] (bob) -- ($(bob)!\Gcos cm!(centro)$) node[left,midway,xshift=-0.75mm,scale=0.50] {$T$};
%
%	draw the angle
%
...
%
     \draw[densely dotted] (bob) arc(-60:-120:3);
     \coordinate (bob1) at ($(mary) + (0,{0.150/2})$);
     \filldraw [densely dotted, fill=black!10,draw=black] (bob1) circle[radius=0.150];
     \path (bob1) node[scale=0.50] {$m$};
     \coordinate (bob2) at ($(bob) + (-3,{0.150/2})$);
     \draw [dashed] (centro) -- (bob2);
     \filldraw [densely dotted, fill=black!10,draw=black] (bob2) circle[radius=0.150];
     \path (bob2) node[scale=0.50] {$m$};
%
%	make sure bob at right position is on top, so draw last
%
     \filldraw [fill=black!20,draw=black] (bob) circle[radius=0.150];
     \path (bob) node[scale=0.60] {${\bf m}$};
   \end{tikzpicture}
  }																							% end \resizebox
