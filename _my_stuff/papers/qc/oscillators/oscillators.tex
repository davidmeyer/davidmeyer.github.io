...
frequently stated as\footnote{It seems to be (a confusing)
convention to use $x$, $x(t)$, $\mathbf{x}$, $\mathbf{x}(t)$,
etc. interchangeably.}  $F \propto -x$. That is, the force $F$ is
proportional to the displacement $x$ (hence Hooke's Law describes
simple harmonic motion aka simple harmonic oscillation; note also
that both $F$ and $x$ are vectors so perhaps we should be using
$\mathbf{F}$ and $\mathbf{x}$). More frequently this relationship
is converted into an equality by the use of a constant $k$ so
that we have
...
\medskip
\noindent
(here $a$ is a vector so perhaps we should be using $\mathbf{a}$).
...
we set $F_{m} = kx$ we get the curve shown in Figure
...
%
       ...
       \draw[ultra thick,-latex](Ystart) -- (Yend) node[above,scale=1.75] {$F_{m}$};	% y axis
...
\bigskip
\noindent
So now we know that $E_{\text{total}} = \frac{1}{2}mv^2 + 
...
&=& \dfrac{1}{2}mv^2 + \dfrac{1}{2}kx^2 									
						&\hspace{4em} \mathrel{\#} E_{\text{total}}
                                                = \text{KE} + \text{PE} \\
[10pt]
...
&\Rightarrow& E_{\text{total}} \propto f^2 A^2 
						&\hspace{4em} \mathrel{\#} \text{$E_{\text{total}}$ is proportional to $f^2 A^2$}\\
...
\bigskip
\noindent
Interestingly, $E_{\text{total}}$ has an important property: it is 
conserved. Section \ref{subsec:conservation_of_energy} explores 
this property.

\subsection{Conservation of Energy}
\label{subsec:conservation_of_energy}
\noindent
Consider the case of a general force function $F(x)$. Here we define 
the kinetic energy of the system to be $\frac{1}{2}mv^{2}$. We also 
define the potential energy of the system, $V(x)$, as the function
...
\medskip
\noindent
Then the total energy of the system as a function of displacement and velocity, 
$E(x,v)$, is defined to be 
...
(\ref{eqn:V(x)}) and (\ref{eqn:E}). Then the total energy of
the system, $E(x(t),\dot{x}(t))$, is conserved.

\bigskip
\noindent
{\bf Proof:} One way to prove that energy is independent of time
is to show that $\dfrac{d}{dt}E(x,\dot{x}) = 0$:
...
&=& \dfrac{d}{dt} \bigg [ \dfrac{1}{2}m(\dot{x}(t))^{2} \bigg ] + \dfrac{d}{dt} V(x(t))
		&\hspace{1em} \mathrel{\#} \text{derivative is a linear 
		                                 operator \cite{wiki:linearity_of_differentiation}} \\
[15pt]
&=& m\dot{x}(t)\ddot{x}(t) + \dfrac{d}{dt} V(x(t))
		&\hspace{1em} \mathrel{\#} \text{by the power rule \cite{wiki:power_rule}} \\
[15pt]
&=& m\dot{x}(t)\ddot{x}(t) + \dfrac{d}{dt} V(u)
		&\hspace{1em} \mathrel{\#} u = x(t), \, du = dx \\
[15pt]
&=& m\dot{x}(t)\ddot{x}(t) + \left [\dfrac{d}{du}  V(u) \right ] \dot{x}(t)
		&\hspace{1em} \mathrel{\#} \text{by the chain rule \cite{wiki:chain_rule}: 
								   $\dfrac{d}{dt} V(u) = 
