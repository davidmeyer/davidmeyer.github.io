...
{\setstretch{1.5}
\noindent
This is a much simpler differential equation which we know how
...
&=& i^2 \omega^2 e^{i\omega t}						
		...
		$\dfrac{d}{dt} \Big ( i \omega e^{i\omega t} \Big ) = (i \omega) \cdot \dfrac{d}{dt} e^{i\omega t} 
...
where $A, B \in \mathbb{C}$ are constants defined by the initial
...
\smallskip
\noindent 
Finally, note that pendulums with small angles are not uncommon.
For example, many clocks have long, thin pendulums: essentially
large $L$ and small $\theta_{\text{max}}$. In practice these two
constants are used produce clocks with long, accurate periods
...
values into Equation \ref{eqn:kirchhoffs_voltage_law}
to get 

\begin{equation*}
V = L \dot{I} + \dot{V}_{C} = L \dot{I} + \bigg (\dfrac{1}{C} \bigg ) I
\end{equation*}
...
[12pt]		
&\Rightarrow& \dfrac{d^2 I}{dt^2} = - \omega^2 I						&\qquad \mathrel{\#} \text{set $\omega = \sqrt{\dfrac{1}{LC}}$}
\end{array}
\end{equation*}

\bigskip
\noindent
What we can notice is that $\dfrac{d^2 I}{dt^2} = - \omega^2 I$ is 
of the same general form as the equation of motion for both the 
...
\bigskip
\noindent
If we add a resister to the series LC circuit
...
\begin{equation*}
V = L \dot{I} + R I + \bigg (\dfrac{1}{C} \bigg ) I
...
{\setstretch{1.75}
\noindent
Taking the derivative of the left side and the
first two terms on the right side to get
all of the terms in the same form and recalling that
...
\bigskip
\noindent
To see this, notice that the equation of motion 
...
\noindent
where $\bigg (\dfrac{b}{m} \bigg ) \dot{x}$ is the new term 
that represents the damping due to friction. We can see that
Equation \ref{eqn:damped_spring_and_mass} has the same form
as Equation \ref{eqn:damp_general_form} and that in general 
the damping force is proportional to $\dfrac{dx}{dt}$.
...
\medskip
\begin{equation}
\ddot{\theta} \approx  - b \dot{\theta} - \dfrac{g}{L} \theta
...
\bigskip
\noindent
Finally, the analogy between the mechanical
and the electrical systems looks like
...
\begin{tabular} {| c | c  |}
\hline
{\Large \bf Mechanical}  & {\Large \bf Electrical}   \\
\hline \hline
Mass   & Coil (Inductor) \\
\hline
Damper & Resistor \\
...
comments and suggestions on Figure \ref{fig:simple_pendulum}. 
