...
notation to represent the time derivative of $x$, i.e.
$\dot{x} = \dfrac{dx}{dt}$, $\ddot{x} = \dfrac{d^2 x}{dt^2}$, 
and so on.} 
is \cite{differential_equations_rlc_circuits}:
...
\bigskip
\noindent
So the equation of motion for the RLC circuit is
...
\bigskip
\noindent
The general form of the equation of motion for
damped harmonic oscillators is 
...
&\Rightarrow& ma + b \dot{x} + kx = 0
		&\qquad \qquad \mathrel{\#} \text{subtract $-b \dot{x} - kx$ from both sides} \\
[14pt]
...
\bigskip
\noindent
So we can see that equation of motion 
...
Interestingly the resulting equation of motion, Equation \ref{eqn:damped_spring_and_mass},
is of the same form as Equation \ref{eqn:damp_general_form}.
The displacement function for the damped harmonic oscillator
...
to Dave Plonka for his comments on Table \ref{tab:analogies}.
