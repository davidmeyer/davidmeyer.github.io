...
\cite{oscillations_and_waves}, but what about PE, the potential 
energy of the system?


\medskip
\noindent
It turns out that the potential energy at a position $x$ is 
equal to the area under the force function curve between the 
equilibrium point (usually the origin) and $x$. Figure 
\ref{fig:PE} attempts to explain why $\text{PE} = \frac{1}{2}kx^2$ 
(where $x := \norm{\mathbf{x}}$) for the force function 
$\mathbf{F(x)} = -k\mathbf{x}$ when only the magnitude of $\mathbf{F(x)}$ 
is considered. That is, $F(x) = kx$. 
...
%
       ...
       \draw[ultra thick,-latex](Ystart) -- (Yend) node[above,scale=1.75] {$F(x)$};      % y axis
       \draw [draw=none,fill=gray!15] (O) -- (EP) -- (X) -- cycle;
       \draw [draw=none] (O) -- (X) node[midway,above,scale=1.35,yshift=0.250cm,xshift=0.80cm] 
       				{${\text{PE} = \text{A} = \frac{1}{2}kx^2}$};
       \draw[very thick,red](O) -- (EP) node [left,midway,xshift=-0.20cm,yshift=0.25cm,scale=1.35] 
       				{${\color{black}{\text{slope} = k}}$}; 
