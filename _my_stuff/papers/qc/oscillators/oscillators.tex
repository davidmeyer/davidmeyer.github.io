...
a scalar quantity \cite{elastic_potential_energy} we only interested 
in the magnitude of $F$, so in this case $F = kx$ (the sign is the 
direction of the $F$ vector in the one-dimensional case, so 
the magnitude of $F$ equals $|F| = |-kx| = kx$). 
Figure \ref{fig:PE} is a graph of $F = kx$ where we can see that the 
area under the curve is $\frac{1}{2} kx^2$. Thus the work done or 
potential energy stored is $\frac{1}{2} kx^2$. 
\par}
%
