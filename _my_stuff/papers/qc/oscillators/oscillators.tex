...
%	      
	      ...
	      \draw [draw=none] (12.00,0.00) -- (12.00,0.00) node[draw,rectangle,blue,midway,yshift=-0.25cm] 
...
\bigskip
\noindent
But here's an interesting problem. Unlike the second order
differential equation of motion for the Hooke's Law spring and
mass problem (Equation \ref{eqn:d2xdt2}), we can't solve the
second order differential equation of motion for the simple
pendulum case (Equation \ref{eqn:d2_theta}). However, maybe we
can solve a different but related problem which I'll call the
"Small Angle Pendulum Problem".
...
to solve. In order to solve this differential equation, we first
notice that that the second derivative $\dfrac{d^2 \theta}{dt^2}$
is the function itself scaled by the constant $- \omega^2$.  This
form is known as a second order linear homogeneous differential
equation \cite{tseng_second_order} and we can guess the function
that exhibits this behavior. It's the exponential function: \par}
...
\dfrac{d \theta}{dt} &=& i \omega e^{i\omega t}		
					&\qquad \qquad \mathrel{\#} \text{$\theta = e^{i\omega t}$ and $\dfrac{d}{dt} e^u = e^u \dfrac{du}{dt}$ with $u =i \omega t$} \\
[12pt]
\dfrac{d^2 \theta}{dt^2} 
&=& \dfrac{d}{dt} \Big [ i \omega e^{i\omega t} \Big ]	
					&\qquad \qquad \mathrel{\#} \text{second derivative is the derivative of the first derivative} \\
[12pt]
&=& i^2 \omega^2 e^{i\omega t}						
					&\qquad \qquad \mathrel{\#} \text{second derivative} \\
[12pt]
&=& - \omega^2 e^{i\omega t}						
...
&=& C_1 i \omega e^{i \omega t} (i \omega) - C_2 i \omega  e^{-i \omega t} (-i \omega)
		&\qquad \qquad \mathrel{\#} \text{$\dfrac{d}{dt} e^u = e^u \dfrac{du}{dt}$ with $u =i \omega t$ and $i^2 = -1$} \\
[10pt]
...
[10pt]
&=& - \omega^2 \theta
		&\qquad \qquad \mathrel{\#} \text{$\theta = C_1 e^{i \omega t} + C_2 e^{-i \omega t}$ (Equation \ref{eqn:theta_sum})} \\
[10pt]
...
period does note rely on the maximum angular displacement of the
pendulum, which is in accordance with Galileo’s observations as
described in \cite{helden1995}.

\subsubsection{Large-Angle Pendulums}
\label{subsubsec:large_angle_pendulums}
TBD


\subsection{The RLC Circuit Oscillator}
...
\caption{Simple RLC Circuit}
\label{fig:simple_rlc_circuit}
\end{figure}
