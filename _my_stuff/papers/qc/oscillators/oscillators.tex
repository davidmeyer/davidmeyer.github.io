...
in Equations (\ref{eqn:V(x)}) and (\ref{eqn:E(x,v)}). Then the total 
...
not change with time, that is, we want to show that $\dfrac{d}{dt}E(x(t),\dot{x}(t)) = 0$.
Computing this derivative requires some machinery from both the 
chain and power rules \cite{wiki:chain_rule,wiki:power_rule}:
\par}
\smallskip
%
%
...
&=& m\dot{x}(t)\ddot{x}(t) + \dfrac{d}{dt} V(x(t))
		&\hspace{0em} \mathrel{\#} \text{by the chain \& power rules:
...
[10pt]
&=& 0
		&\hspace{0em} \mathrel{\#} \text{by Newton's Second Law: $m \ddot{x}(t) - F(x(t)) = 0$}
\end{array}
\end{equation*}
...
of motion}), since a particle neither gains nor loses energy 
