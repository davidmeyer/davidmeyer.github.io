...
frequently stated\footnote{It seems to be convention to use $x$ 
and $x(t)$, $F$, $F(x)$ and $F(x(t))$, and $a$ and $a(t)$
interchangeably.}as $F \propto -x$, that is, the force $F$ is
...
use of a spring constant $k$ so that we have
...
\bigskip
\noindent
Setting Hooke's force (Equation (\ref{eqn:hookes_law})) equal
to Newton's force (Equation (\ref{eqn:newtons_2nd_law}))  
gives us
...
%	      
	      ...
	      \draw [draw=none] (12.00,2.00) -- (12.00,2.00) coordinate [label={[midway] \underline{{\bf Hooke's Force = Newton's Force}}}];
