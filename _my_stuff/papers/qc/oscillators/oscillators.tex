...
frequency (in radians per second). We also know that the definition of angular
frequency is
...
frequency in cycles/second \cite{wiki:angular_frequency}. 
Note that $f = \dfrac{1}{T}$ so $f$ has units of $\text{seconds}^{-1}$.  

\bigskip
\noindent
Give this information we can write
the displacement function $x(t)$ as 
in terms of the frequency $f$ as follows:
