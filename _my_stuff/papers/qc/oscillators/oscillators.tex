...
period ($T$). The period depends on the length of the pendulum and also
to a slight degree on the amplitude ($A$), the width of the pendulum's
swing. The free body diagram \cite{wiki:free_body_diagram} for the 
simple pendulum is shown in Figure \ref{fig:simple_pendulum}.
...
%
     \draw[red,densely dotted] (bob) arc(-60:-120:3);
     \coordinate (bob1) at ($(mary) + (0,{0.150/2})$);
     ...
     \draw [dashed] (centro) -- (bob2);
%
%	draw the free body diagram at bob2 (or not)
%
%     \draw [thick,brown,-stealth] (bob2) -- ++(0,-\Gvec) coordinate (g2)
%       node[below,brown,yshift=-1.0mm, scale=0.60] {${\bf F_{g}} = m {\bf g}$};
%     \draw [->] (bob2) -- ($(bob2)!\Gsin cm!255:(centro)$) coordinate (gsin2);
%     \draw [->] (bob2) -- ($(bob2)!-\Gcos cm!(centro)$)    coordinate (gcos2); 
%     \pic [draw, ->, "{$\Scale[0.60] {\theta}$}", angle eccentricity=1.3] {angle = g2--bob2--gsin2};
%     \draw [densely dotted, thin, blue] (g2) -- (gsin2) 
%       node[midway, xshift=0.15cm, rotate=60, scale=0.50] {$\color{black}{mg \sin \theta}$};
%     \draw [densely dotted, thin, blue] (g2) -- (gcos2) 
%       node[midway,xshift=-0.35mm, yshift=-0.125cm,rotate=-30, scale=0.50] {$\color{black}{mg \cos \theta}$};  
%
%
%
...
%
     ...
     \filldraw [fill=black!15,draw=black] (bob) circle[radius=0.150];
     \path (bob) node[scale=0.50] {${\bf m}$};
     
     \filldraw [very thin, fill=black!5,draw=black] (bob2) circle[radius=0.150];
...
Thanks to Chris Lonvick (lonvick@gmail.com) for his comments and help on/with Figure \ref{fig:simple_pendulum}.
