...
\begin{equation}
\int_{a}^{b} f(t) \, dt = re^{i\theta}
\label{equation:integral_polar_form}
\end{equation}


\noindent
for some constants $r, i$ and $\theta$.


\bigskip
\noindent
Why does this hold? First, recall that any complex number $z$ 
can be represented as $z = a + bi$. Then consider the complex 
plane, shown in Figure \ref{fig:complex_plane}:
...
&=& \left | re^{i\theta} \right |
	&\hspace{3em} \mathrel{\#} \text{take the absolute value of both sides of Equation (\ref{equation:integral_polar_form})} \\
[8pt]
&=& |r||e^{i\theta}|
...
[8pt]
&=& r
	&\hspace{3em} \mathrel{\#} \text{$r \in \mathbb{R}$ and $r > 0$} \\
\end{array}
\end{equation*}
...
\bigskip
\noindent
Now, if we look at the real part of the integral we see
that

\begin{equation*}
\begin{array}{llll}
{\displaystyle \operatorname{Re} \int_{a}^{b} e^{-i\theta} f(t) \, dt} 
&=& {\displaystyle \int_{a}^{b} \operatorname{Re} \left [e^{-i\theta} f(t) \right ] dt}
	&\hspace{3em} \mathrel{\#} \text{swap Re and $\int$, noting that $\left [e^{-i\theta} f(t) \right ] \in \mathbb{C}$} \\
[10pt]
&\leq& {\displaystyle \int_{a}^{b} \left | e^{-i\theta} f(t) \right | \, dt}
	&\hspace{3em} \mathrel{\#} \text{since $\operatorname{Re}(z) \leq |z|$ for $z \in \mathbb{C}$} \\
[10pt]
&=& {\displaystyle \int_{a}^{b} \left | e^{-i\theta} \right | \left | f(t) \right | \, dt}
	&\hspace{3em} \mathrel{\#} |xy| = |x||y| \\
[10pt]
...
\bigskip
\noindent
So if $s = c$ then $\frac{M}{\mathlarger{s-c}}$ is not defined
and $\mathcal{L}\{f(t)\}$ does not exist. Similarly, if $s < c$
then $\lim\limits_{d \to \infty}\Big [\frac{M}{c-s} e^{(c-s)d}
\Big ]$ does not converge and $\mathcal{L}\{f(t)\}$ does not
exist. All of this implies that functions that do not satisfy the
conditions of the Existence Theorem do not have Laplace
Transforms.
...
such that $\mathcal{L}\{f(t)\} = F(s)$ then the Inverse Laplace
Transform of $F(s)$ is $f(t)$, i.e. $\mathcal{L}^{-1}\{F(s)\} =
f(t)$.
\end{definition}
...
%  \draw [<->] (0,2) -- (0,0) -- (4,0);                                                                      % draw axes
     ...
     \draw [dashed] (0,1) -- (2,1);
     \draw [thick,red] (1,1) circle [radius =0.03cm];                                                % draw an open red circle on the x axis above the "a"              
     \coordinate (y) at (0,1); \fill [black] (y) circle (1pt);                                           % draw a dot on the y axis (I'd like to make it smaller but I don't see how)
     \draw (0,1) node[left] {1};
  \end{tikzpicture}
  \caption{$u(t-a)$}
...
\bigskip
\noindent
Another useful version of the Heaviside function, $u(a-t)$, is shown in Figure \ref{fig:u(a-t)}.
...
\bigskip
\noindent
Now the obvious question is how do we compute
$\mathcal{L}\{u(t-a)\}$? Recall that by the definition of the
...
\bigskip
\noindent
The first thing to observe is that $u(t-a)$ is zero for $t < a$
and one otherwise (Figure \ref{fig:u(t-a)}), so we can rewrite
