...
Group                           & \checkmark & \checkmark & \checkmark & N/A        & no    & Monoid with inverses: $a \in S \Rightarrow a^{-1} \in S$ \\
...
Division Ring                   & \checkmark & \checkmark $(+,*)$      &\checkmark $(+,*)$  & \checkmark
                                             & \checkmark $(+)$  & Ring with inverses: $a \in \text{R}\backslash\{0\} \Rightarrow a^{-1} \in \text{R}$ \\
...
Algebra over a Field            & yes/no & \checkmark $(+,*)$      & \checkmark $(+)$   & \checkmark   & \checkmark $(+)$
                                             & Vector space with bilinear $\text{product}^{\ref{item:bilinear_map}}$  \\
Algebra over a Ring            & yes/no & \checkmark $(+,*)$      & \checkmark $(+)$   & \checkmark   & \checkmark $(+)$
                                             & Module with bilinear product 
\end{tabular}}
...
\bigskip 
\noindent
Matrix multiplication is an example of a bilinear map because it
satisfies the key properties of bilinearity: linearity in each
argument.

\bigskip
\noindent
Consider the multiplication of two matrices $A$ and $B$ where $A
\in M(m,n)$ and $B \in M(n,p)$, producing a matrix $C = AB \in
M(m,p)$. Matrix multiplication is linear in the first argument
because for any matrices $A_1, A_2 \in M(m, n)$ and scalar
$\alpha$ we have $(\alpha A_1 + A_2)B = \alpha A_1B +
A_2B$. Similarly, matrix multiplication is linear in the second
argument because for any matrices $B_1, B_2 \in M(n, p)$ and
scalar $\alpha$ we have $A(\alpha B_1 + B_2) = \alpha AB_1 +
AB_2$.
