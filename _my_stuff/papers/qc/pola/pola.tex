...
point along its path.  The Principle of Least  Action states that the path for which the sum of these differences is minimum is the actual path the particle will take. To show that this is the case
all of the proofs that I have seen rely on the fact that the Principle of Least  Action and Newton's Second Law ($F=ma$) are equivalent. These notes show this result below.
...
\bigskip
\noindent
Now, consider a particle moving in one dimension in a uniform gravitational field $g$ with potential energy $V[x(t)]$\footnote{$V[x(t)]$ is sometimes called $U[x(t)]$ or simply $U(x)$, 
and is also informally referred to as $PE$.}. In this case $V[x(t)] = m \cdot g \cdot x(t)$. So
...
[14pt]
&=& {\displaystyle \mathlarger{ \int_{t_1}^{t_2}} \left [ \frac{1}{2} m \left ( \dfrac{d}{dt} x(t) \right )^2 - V \big  [ x(t) \big ] \right  ] dt}   
                                                                                                              &\qquad \qquad \qquad \qquad \mathrel{\#}  KE = \frac{1}{2} m \left ( \dfrac{d}{dt} x(t)\right )^2\!\!\!\!, \; PE = V  \big  [ x(t) \big ] 
\end{array}
\end{equation*}
...
\bigskip
\noindent
Hopefully simplifying by setting $x = x(t)$ we can see that 
...
{\setstretch{1.35} 
\noindent
$L$ here is called the \emph{Lagrangian}.  The Lagrangian is essentially the action per unit time. An application of the Lagrangian involves a mass on the end of a spring.
In this case we have $T = \dfrac{m}{2} \dot{x}^2$  (where $\dot{x}$ is shorthand for $\frac{dx}{dt}$)  and $V = \frac{k}{2}x^2$. So now we have \par}
...
\section{Solving the Simple Harmonic Oscillator Problem Using the Lagrangian}
Consider the simple harmonic oscillator \cite{wiki:harmonic_oscillator} example shown in Figure \ref{fig:shm}.
Here the mass $m$ is moving back and forth in one dimension on a frictionless surface with a spring constant $k$
and equilibrium point EP.
...
\bigskip
\noindent
The first step is to solve the Euler–Lagrange equation (Equation \ref{eqn:euler-lagrange}) for this value of $L$. Recall that 
...
\bigskip
\noindent
So we first want to compute $\dfrac{\partial L}{\partial x} = \dfrac{\partial}{\partial x}  \bigg [ \dfrac{1}{2} m \dot{x}^2  - \dfrac{1}{2} k x^2 \bigg ]$. 
...
\bigskip
\begin{equation*}
\dfrac{\partial L}{\partial x} = \dfrac{\partial}{\partial x}  \bigg [ \dfrac{1}{2} m \dot{x}^2  - \dfrac{1}{2} k x^2 \bigg ] = \dfrac{\partial}{\partial x} \bigg [\dfrac{1}{2} m \dot{x}^2  \bigg ] - \dfrac{\partial}{\partial x} \bigg [\dfrac{1}{2} k x^2  \bigg ]  =  0 - \dfrac{\partial}{\partial x} \bigg [\dfrac{1}{2} k x^2  \bigg ] = - \dfrac{\partial}{\partial x} \bigg [ \dfrac{1}{2} k x^2  \bigg ]  = - \dfrac{1}{2} 2 k x = -kx
\end{equation*}

\bigskip
\noindent
Next, we need to find  $\dfrac{\partial L}{\partial \dot{x}}$ and then take its derivative with respect to $t$ (time). Here we know that $\dfrac{\partial}{\partial \dot{x}} \Bigg [ \dfrac{1}{2} k x^2 \Bigg ] = 0$
which tells us that 


\begin{equation*}
\dfrac{\partial L}{\partial \dot{x}} = \dfrac{\partial}{\partial \dot{x}} \Bigg [   \dfrac{1}{2} m \dot{x}^2 - \dfrac{1}{2} k x^2 \Bigg ] = \dfrac{\partial}{\partial \dot{x}} \Bigg [   \dfrac{1}{2} m \dot{x}^2 \Bigg ]  -  \dfrac{\partial}{\partial \dot{x}} \Bigg [  \dfrac{1}{2} k x^2 \Bigg ] = \dfrac{\partial}{\partial \dot{x}} \Bigg [  \dfrac{1}{2} m \dot{x}^2 \Bigg ]  - 0
=  \dfrac{\partial}{\partial \dot{x}} \Bigg [  \dfrac{1}{2} m \dot{x}^2 \Bigg ] =  \dfrac{1}{2} 2 m \dot{x} = m \dot{x}
\end{equation*}

\bigskip
\noindent
Now we can see that $\dfrac{d}{dt} \bigg ( \dfrac{\partial L}{\partial \dot{x}} \bigg )   = \dfrac{d}{dt} \big ( m \dot{x} \big ) = m \ddot{x}$. Plugging these back 
values into the Euler–Lagrange equation we find that
...
Here we recognize that $\ddot{x} - \dfrac{k}{m} x = 0$ is a differential equation that has a solution in sine or cosine (doesn't matter which) where $\dfrac{k}{m}$ represents
 the angular frequency. If we let $\omega = \sqrt{\dfrac{k}{m}}$ then $\omega^2 = \dfrac{k}{m}$ and so $\ddot{x} + \omega^2 x = 0$. But this 
 is a differential equation that  we recognize has the general solution \cite{harmonic_oscillartors} \par}
 ...
 \bigskip
 \noindent
 So in this simple system the position of the mass as a function of time is given by $ x(t) = A \cos (\omega t)$ for some amplitude $A$ and where the angular frequency 
 is given by the spring constant ($k$) divided by the mass of the object ($m$).

\section{Conclusions}
It's amazing that two such different approaches, Newton mechanics and Lagrangian mechanics, turn out to be equivalent. BTW, Kenneth Young has a very nice lecture on the Principle
