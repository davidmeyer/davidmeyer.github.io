...
%
\usepackage{tikz}
\usetikzlibrary{calc,patterns,angles,quotes,shapes,intersections,decorations,backgrounds}   
...
\centering
  ...
  \begin{tikzpicture} [every edge quotes/.append style = {anchor=south, sloped}, declare function={f(\x)=\x*\x + 1;}]  % define f(x)
     \draw[thick, ->] (-5,0) -- (5,0) coordinate [label={[xshift=0.30cm, below] $x$}];
     \draw[thick, ->] (0,-1) -- (0,5) coordinate [label={[xshift=-0.30cm, left] $f(x)$}];
     \draw [line width=0.20mm, smooth, samples=100, domain=-1.75:1.75] plot (\x,{f(\x)});
     ...
     \fill (p) circle (0.05);                                           % draw a dot at minimum
%
...
%
     ...
     \draw (5.5,1) node[rectangle, draw, very thin] {$\displaystyle \dfrac{d}{dx} f(x)  = \dfrac{d}{dx} f(x + \epsilon) = 0$};
    \end{tikzpicture}
   }                                                                    % end resizebox
...
\newpage
\noindent
We also know that we can evaluate $f(x + \epsilon)$ using a
Taylor series \cite{wiki:taylor}. In particular 


\begin{equation*}
\begin{array}{lllll}
f(x + \epsilon)
&=&  f(x) + \epsilon \dfrac{df}{dx} + \dfrac{\epsilon^2}{2!} \dfrac{d^2 f}{dx^2} +  \dfrac{\epsilon^3}{3!}  \dfrac{d^3 f}{dx^3} + \cdots
                                                &\qquad \mathrel{\#} \text{definition of the Taylor series for $f(x + \epsilon)$}
\end{array}
\end{equation*}
...
\begin{figure}[H]
\centering
  \resizebox{0.60 \textwidth}{!} {                              % resize the figure if you want
    \begin{tikzpicture}  [every edge quotes/.append style = {anchor=south, sloped}]
       \draw[thick, ->] (-0.35,0) -- (13.5,0) coordinate [label={[xshift=0.30cm, below]  $t$}] (t);
       \draw[thick, ->] (0,-0.35) -- (0,10.5) coordinate [label={[xshift=-0.30cm, left]  $x$}] (x);
...
%
 ...
 %        
       \draw (9.5,2.25) node[draw,very thin,rectangle] 
        {$\mathlarger {\mathlarger {{\displaystyle \; \mathcal{S} \big [ (x(t) \big ]= \int_{t_1}^{t_2} (KE - PE) \; dt \;}}}$}; 
    \end{tikzpicture}
  }                                                                                                     % end resizebox
...
{\setstretch{1.35} 
\noindent
$L$ here is called the Lagrangian.  The Lagrangian is essentially the
...
in Section \ref{sec:simple_harmonic_oscillator}. \par}

\bigskip
\noindent
Here the kinetic energy $T$ is $\frac{1}{2} m \dot{x}^2$ and the
spring potential energy $V,$ given by Hooke's Law
\cite{wiki:hookes_law}, write the Lagrangian as follows: 
...
\dfrac{1}{2} k x^2 \bigg ]$. Since differentiation is a linear
operation \cite{wiki:linearity_of_differentiation} and we know
that $\dfrac{\partial}{\partial x} \bigg [\dfrac{1}{2} m
\dot{x}^2  \bigg ] = 0$ we can see that  
...
\bigskip
\noindent
To start, consider the simple harmonic oscillator
\cite{wiki:harmonic_oscillator} shown in Figure
\ref{fig:shm}. Here the mass $m$ is moving back and forth in one
dimension on a frictionless surface with spring constant $k$ and
...
\centering
  \resizebox{0.65 \textwidth}{!} {                                                              % resize figure if you want
   \begin{tikzpicture}[black!75,thick]
        ...
        \draw[ultra thick] (0,-1) -- (0,5.5) coordinate [label={[]}];
        \draw[draw=none] (-1,0) -- (11,0) coordinate [label={[yshift=-0.3cm,midway,below] 
                                      ${L = KE- PE, \; KE = \frac{1}{2} m v^2  \text{ and } PE = \frac{1}{2} k x^2}$}];
...
%
 ...
 %
      ...
      \draw[->] (6.5,0.40)-- (7.5,0.40) coordinate [label={[right] ${v = \dot{x}}$}]; % draw velocity vector
...
%
      ...
      \draw[dashed] (c1) -- (c2) coordinate [label={[above] \text{\underline{Equilibrium Point}}}];   
      \draw[<-] (4,2.5) -- (5,2.5) coordinate [label={[right] $\vec{F} = -kx$}];   
      \fill (c1) circle(0.05);      
    \end{tikzpicture}
  }                                             % end resizebox
 \caption{Simple Harmonic Oscillator with Spring Constant $k$ and Mass $m$}
 \label{fig:shm}
\end{figure}
