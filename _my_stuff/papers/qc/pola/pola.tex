...
Here we recognize that $\ddot{x} + \dfrac{k}{m} x = 0$ is a differential equation that has a solution in sine or cosine (doesn't matter which) where $\dfrac{k}{m}$ represents
 ...
 \bigskip
 \noindent
 where $A$ is the amplitude, $\omega_0$ is the angular frequency and $\delta$ is phase. Note that $A$ and $\delta$ depend on initial conditions while  $\omega_0$ 
 does not. 
 
 \bigskip
 \noindent
 So in this simple system the position of the mass as a function of time is given by $ x(t) = A \cos (\omega_0 t - \delta)$ for some amplitude $A$ and where the angular frequency 
