...
&=& {\displaystyle \mathlarger{ \int_{t_1}^{t_2}} \left [ \frac{1}{2} m \left ( \dfrac{dx}{dt} \right )^2 - V(x) \right  ] dt}   
                                                                                                              & \qquad \mathrel{\#}  KE = = \frac{1}{2} m v^2 = \frac{1}{2} m \left ( \dfrac{dx}{dt} \right )^2\!\!\!\!, \; PE = V(x) = m \cdot g \cdot x \\
\end{array}
\end{equation*}
...
\bigskip
\noindent
Equation \ref{eqn:euler-lagrange} is called the \emph{Euler-Lagrange} equation. For the mass on the end of a spring problem we can see from Equation \ref{eqn:moeos}
that $\frac{\partial L}{\partial \dot{x}} = m\dot{x}$ and $\frac{\partial L}{\partial x} = -kx$ so Equation \ref{eqn:euler-lagrange} give us
...
\bigskip
\noindent
The integral of $L$ between two points $t_1$ and $t_2$ gives us the action of the path, which we saw in Equation \ref{eqn:S(x)}.  

\bigskip
\noindent
If we combine the Lagrangian (Equation \ref{eqn:lagrangian}) with the Euler-Lagrange equation (Equation \ref{eqn:euler-lagrange}) we get
...
Equations \ref{eqn:lagrangian} and \ref{eqn:euler-lagrange} (written in different notation) together again 
tell us that $F = ma$.
