...
[4pt]
&=& 0                                                                        &\mathrel{\#} \text{$\dfrac{df}{dx} = 0$ at the extremum} \\
\end{array}
\end{equation*}
...
of Least Action looks at the value of an entire  \emph{path}, where as Newton's Second Law looks at points along the path. More specifically, in Newton's case we 
are given initial conditions (the initial position and the initial velocity) and then we solve the differential equation (Newton's second law, $F=ma$). Put another way,
we want to solve $- \dfrac{\partial V}{\partial x} = m \dfrac{d^2x}{dt^2}$ to find $x(t)$. Note that  $\dfrac{d^2x}{dt^2}$ is frequently written as $\ddot{x}$. As we will see in Section
\ref{sec:pola},  unlike in Newtonian mechanics in Lagrangian mechanics (that is, the Principle of Least Action) we find an object's trajectory $x(t)$ from its beginning 
and ending positions. Amazingly these two approaches turn out to be  equivalent. \par}
...
{\setstretch{1.35} 
\noindent
$L$ is called the \emph{Lagrangian}.  The Lagrangian is essentially the action per unit time and as we saw above,  in classical mechanics this particular 
...
\bigskip
\noindent
For our discussion here we have $T = \dfrac{m}{2} \dot{x}^2$  (where $\dot{x}$ is shorthand for $\frac{dx}{dt}$)  and $V = \frac{k}{2}x^2$. 
So the Lagrangian for this case is
...
\bigskip
\noindent
Essentially the Euler-Lagrange equation is a condition that the Lagrangian has to satisfy in order for the principle of stationary action to be true. It is essentially what 
generates the equations of motion of a system given a specific Lagrangian, just as Newton’s second law does for a given force.
...
that from Equation \ref{eqn:moeos} that $\frac{\partial L}{\partial \dot{x}} = m\dot{x}$, $\frac{d}{d t}  m\dot{x} = m\ddot{x}$ and $\frac{\partial L}{\partial x} = -kx$. Combining these results in 
