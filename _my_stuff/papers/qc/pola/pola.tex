...
are given initial conditions (the initial position and the initial velocity) and then we integrate Newton's second law ($F=ma$) to find $x(t)$. As we will see in Section
\ref{sec:pola},  unlike in Newtonian mechanics in Lagrangian mechanics (that is, the Principle of Least Action) we find $x(t)$ from an object's beginning 
and ending positions. Amazingly these two approaches turn out to be  equivalent.
