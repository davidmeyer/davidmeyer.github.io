...
someone who is not moving with the clock \cite{physics:spacetime_hughes}.
...
\bigskip
\noindent
Said another way, special relativity tells us that for an observer $O$
in an inertial frame of reference, a clock that is moving relative to 
the observer will be measured to tick more slowly than a clock at rest 
in the observer's frame of reference \cite{wikipedia:time_dilation}. 
...
\footnote{Assume here that the thickness of the source and the detector 
are negligible so that the distance light travels is $h$.}. Here the 
observer, light source and detector are at rest in inertial frame of 
reference $I$ (even though the train car is moving with velocity $v$).
...
%
        ...
        \draw[draw=none] (6,3) -- (6,0) node[below, right] {$I$};
%
...
\begin{equation*}
  \begin{array}{llll}
     h
     &=& c \Delta t_0 					
     		&\hspace{2em} \mathrel{\#} \Delta t_0 
		                   \text{ is the time for light to travel from the 
...
%
        ...
        \draw[thick] (0.5,-0.5) circle (0.5cm);                                 % left wheel
        \draw[thick] (5.5,-0.5) circle (0.5cm);                                 % right wheel
        \draw[draw=none] (6,3) -- (6,0) node[below, right] {$I^{\prime}$};      % label frame of reference
%
%       Fix this
%
%
...
from the source to the detector. $\Delta x$ is the distance the
train moves forward in time $\Delta t$. We also know that the speed of
light in a vaccum is $c$ for all observers, so distance that the light
travels in this case is $c \Delta t$ (the hypotenuse of the triangle
show in Figure \ref{figure:observer_motion}). 

\bigskip
\noindent
The $h$, $\Delta x$, $c \Delta t$ triangle is abstracted in 
Figure \ref{figure:triangle}. 
...
\begin{figure}[H]
  ...
  \resizebox{0.35 \textwidth}{!} {                                                                      % resize the figure if you want
     \begin{tikzpicture} 
        ...
        \draw (1,1) -- (3,1) -- (1,4) -- cycle; 
        \draw[draw=none] (1,1) -- (1,4) node [midway,left] {$h$};
        \draw[draw=none] (1,1) -- (3,1) node [midway,below] {$\Delta x$};
        \draw[draw=none] (1,4) -- (3,1) node [midway,right,xshift=0.15cm] {$d = c \Delta t$};
        \end{tikzpicture}
     }
  \caption{Triangle formed by the motion of $I^{\prime}$ relative to $I$}
  \label{figure:triangle}
\end{figure}

\bigskip
\noindent
Using this triangle we can calculate $\Delta t$ using 
Pythagorean theorem:
...
[15pt]
	&\Rightarrow& (c \Delta t)^2 = (c \Delta t_{0})^2 + (v \Delta t)^2
			&\hspace{2em} \mathrel{\#} \text{velocity $v$ of the train: } 
			    v = \dfrac{\Delta x}{\Delta t} \Rightarrow \Delta x = v \Delta t \\
[15pt]
...
\bigskip
\noindent
So for observer $O$ the passage of time on the moving train slows down by a factor of
...
factor (or sometimes the gamma factor). Note also that $\Delta t$ 
can be written using the Lorentz factor: $\Delta t = \gamma \Delta t_0$.
%
%
%
