...
to the train car (which is moving with velocity $v$).\footnote{Note that the
velocity $v$ here is a the real constant with dimensions $\dfrac{\Delta x}{\Delta t}$,
representing a velocity 
confined to the x-direction.} \par}
...
train moves forward in time $t^{\prime}$. We also know that the speed of
light in a vaccum is $c$ for all observers, so the distance that light
travels (from the source to the detector) in this case is $c t^{\prime}$ 
(the hypotenuse of the triangle shown in Figure \ref{figure:observer_motion}). 

\bigskip
\noindent
The $h$, $\Delta x$, $c t^{\prime}$ triangle is abstracted in 
...
[15pt]
     &\Rightarrow& (c t^{\prime})^2 = {(ct)}^2 + (\Delta x)^2 					
     		&\hspace{1.25em} \mathrel{\#} h = c t \\
[10pt]
	&\Rightarrow& (c t^{\prime})^2 = (ct)^2 + (v t^{\prime})^2
			&\hspace{1.25em} \mathrel{\#} \text{relative speed $v$ of the train: } 
			    v = \dfrac{\Delta x}{t^{\prime}} \Rightarrow \Delta x = v t^{\prime} 
			    \text{ (note: $v$ is not a vector)} \\
[15pt]
...
[15pt]
	&\Rightarrow& {t^{\prime}}^2 = t^2 + \dfrac{v^2}{c^2} {t^{\prime}}^2
			&\hspace{1.25em} \mathrel{\#}  \text{divide both sides by $c^2$} \\
[15pt]
	&\Rightarrow& {t^{\prime}}^2 - \dfrac{v^2}{c^2} {t^{\prime}}^2 = t^2 
			&\hspace{1.25em} \mathrel{\#}  \text{subtract $\dfrac{v^2}{c^2} {t^{\prime}}^2$ from both sides} \\
[15pt]
	&\Rightarrow& {t^{\prime}}^2 \left (1 - \dfrac{v^2}{c^2} \right) = t^2 
			&\hspace{1.25em} \mathrel{\#} \text{factor out ${t^{\prime}}^2$} \\
[15pt]	
	&\Rightarrow& {t^{\prime}}^2 = \dfrac{t^2}{\left (1 - \dfrac{v^2}{c^2} \right)}
		&\hspace{1.25em} \mathrel{\#}  \text{divide both sides by $\left (1 - \dfrac{v^2}{c^2}\right )$} \\
[35pt]
	&\Rightarrow& t^{\prime} = \dfrac{t}{\sqrt{1 - \dfrac{v^2}{c^2}}}
		&\hspace{1.25em} \mathrel{\#}  \text{take the square root of both sides}
  \end{array}
\end{equation*}
...
Note also that $t^{\prime}$ can be written using the Lorentz factor: 
$t^{\prime} = \gamma t$. The Lorentz factor is sometimes written
$\gamma(v)$ to emphasize $v$.
