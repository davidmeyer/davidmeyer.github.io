...
\begin{figure}[H]
  ...
  \resizebox{0.35 \textwidth}{!} {                                                                      % resize the figure if you want
     \begin{tikzpicture} 
        ...
        \draw[draw=none] (1,4) -- (3,1) node [midway,right,xshift=0.15cm] {$c \Delta t$};
        \end{tikzpicture}
     }
...
\bigskip
\noindent
So for observer $O$ the passage of time on the moving train slows down by a 
factor of $\gamma = \dfrac{1}{\sqrt{1-\dfrac{v^2}{c^2}}}$.

\bigskip
\noindent
$\gamma$ is called the Lorentz factor (or sometimes the gamma factor). 
Note also that $\Delta t$ can be written using the Lorentz factor: 
