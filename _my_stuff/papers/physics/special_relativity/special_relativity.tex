...
\medskip
\remark{\bf The meaning of $v$}:
\label{remark:the_meaning_of_v}
Even though the word "velocity" is used to characterize the variable $v$,
$v$ usually refers to the magnitude of the relative velocity between two 
inertial reference frames, not a velocity vector.
...
the train landing. That is, the observer is at rest with respect
to the train car (which is moving with velocity $v$).
...
%
        \draw[->,dashed, thick, red] (2.2,2.7) -- (4.0,0.2) node [midway,right,xshift=2.0mm] {$\color{black}{c\Delta t}$};   
...
light in a vaccum is $c$ for all observers, so the distance that light
travels (from the source to the detector) in this case is $c \Delta t$ 
(the hypotenuse of the triangle shown in Figure \ref{figure:observer_motion}). 
...
\bigskip
\noindent
Note that the variable $v$ in the expression for $\gamma$ represents the magnitude 
of the relative velocity between two inertial reference frames, not a velocity 
vector.
