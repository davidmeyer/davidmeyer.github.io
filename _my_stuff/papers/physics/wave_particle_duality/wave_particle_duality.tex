...
\begin{theorem}
Suppose a particle satisfies Newton’s Second Law in the form $m\ddot{x} = F(x)$. 
Let $V$ and $E$ be as in Equations (\ref{eqn:V(x)}) and (\ref{eqn:E}). Then the 
energy $E$ is conserved. Said another way: For each solution $x(t)$ of Newton’s 
Second Law, $E(x(t),\dot{x}(t))$ is independent of $t$.
...
&=& \dfrac{d}{dt} \bigg [\dfrac{1}{2}m(\dot{x}(t))^{2} + V(x(t)) \bigg ]	
		&\hspace{1em} \mathrel{\#} \text{Equation (\ref{eqn:E}) with $x = x(t)$ and $v = \dot{x}(t)$} \\	
[15pt]
...
[15pt]
&=& m\dot{x}(t)\ddot{x}(t) + \dfrac{d}{du} V(u) \, \dot{x}(t)
