...
\begin{flalign}
\label{eqn:score-fn}
\E_{x \sim p_{\theta} (x)}\big [\nabla_{\theta} \log p_{\theta}(x) \big] & = \int p_{\theta}(x) \nabla_{\theta} \log p_{\theta}(x) dx   \quad \mathrel{\#} \text{definition of expectation} \\
...
\begin{array}{llll}
V^{\pi}(s_t) 
&=& \E_{s_{t+1} \sim p(s_{t+1} | s_t, u_t), u_t \sim
\pi(u_t|s_t)}\Bigg [ \sum\limits_{l = 0}^{H - 1} r_{t+l} \Bigg ]
		&\mathrel{\#} \text{finite horizon state value function} \\ 
[15pt]
Q^{\pi}(s_t,u_t) 
&=& \E_{s_{t+1} \sim p(s_{t+1} | s_t, u_t), u_{t+1} \sim \pi(u_{t+1} | s_{t+1})} \Bigg [
\sum\limits_{l = 0}^{H - 1} r_{t+l} \Bigg ]
		&\mathrel{\#} \text{finite horizon state-action value function} \\
[20pt]
A^{\pi}(s_t,u_t) 
&=& Q^{\pi}(s_t,u_t) - V^{\pi}(s_t) 
