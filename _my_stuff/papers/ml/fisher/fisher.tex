...
think about the Fisher Information is that it is frequently (if not 
always) defined to be the variance of the score function 
$\nabla_{\theta} \log p_{\theta}(X)$. 
Recall that the mean $\mu = \mathbb{E}[X]$ and the variance 
$\sigma^2 = \mathbb{E} \left [{ \big (X - \mu \big )}^2 \right ]$. 


\bigskip
\noindent
Now, consider the mean of the score function 
$\mathbb{E} \left [\nabla_{\theta} \log p_{\theta}(X) \right ]$:  \par}
...
$\mathbb{E}_{\theta}\big[\nabla_{\theta} \log p_{\theta}(X)\big]
= \mathbb{E}_{\theta} \big[l_{\theta} \big ] = 0$. This condition (expected value is zero) is the
