...
\bigskip
\noindent
So far we have the following forms of the Fisher Information
$\mathbb{I}_\theta$: 
...
&=& - {\displaystyle \int p_\theta(x) \nabla^2 \log p_\theta(x) \; dx + 
					 \int \nabla^2 p_\theta(x) \; dx} 
		&\qquad \qquad \mathrel{\#} \dfrac{p_\theta(x)}{p_\theta(x)} = 1 \\ 
[15pt]
&=& -\mathbb{E}_{\theta} \big [\nabla^2 \log p_\theta(x) \big] + 
        			\nabla^2 \underbrace{\int p_\theta(x) \; dx}_{1}
		&\qquad \qquad \mathrel{\#} \text{$p_\theta(x)$ is a density} \\ 
[15pt]
&=& -\mathbb{E}_{\theta} \big [\nabla^2 \log p_\theta(x) \big] + \nabla^2 1 
		&\qquad \qquad \mathrel{\#} {\displaystyle \int p_\theta(x) \; dx = 1} \\ 
[18pt]
...
\begin{array}{llll}
\mathbb{I}_p 
&=& \mathbb{E}_p \Bigg [\bigg (\frac{X}{p} - \Big(\frac{1-X}{1-p} \Big) \bigg)^2\Bigg] \\
[15pt]
&=&\frac{1}{p} + \frac{1}{1 - p} \\
[15pt]
&=& \frac{1}{p (1-p)}
...
&=& \mathrm{Var}(X)
		&\qquad \qquad \mathrel{\#} \text{definition of variance} \\ 
[10pt]
&=& p(1-p)
		&\qquad \qquad \mathrel{\#} \text{variance of the Bernoulli distribution
		                                  \cite{wiki:bernoulli_distribution}} \\ 
[10pt]		
&=& \frac{1}{\mathbb{I}_p}  
		&\qquad \qquad \mathrel{\#} \text{Equation (\ref{eqn:fisher}): }
		                            p(1-p) = \frac{1}{\mathbb{I}_p} \\
\end{array}
\end{equation*}
