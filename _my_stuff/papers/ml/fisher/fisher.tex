...
Fisher information associated with the \emph{model} is the matrix
given by
...
\log p_{\theta}(X)$. The score function turns up in perhaps
...
\bigskip
\noindent
Fisher Information can be described in various ways. One way to
think about the Fisher Information is that it is the variance of
the score function $l_\theta$. Recall that the mean $\mu =
\mathbb{E}[X]$ and the variance $\sigma^2 = \mathbb{E}[(X -
\mu)^2]$. First, consider $\mathbb{E}\big[\nabla_{\theta} \log
p_{\theta}(X)\big] = \mathbb{E}\big[l_\theta\big]$.


\begin{equation*}
\begin{array}{llll}
\mathbb{E}_{\theta}\big [l_\theta\big] 
...
&=& {\displaystyle \int \nabla_\theta p_\theta(x) dx} 
		&\qquad \qquad \mathrel{\#} \frac{p_\theta(x)}{p_\theta(x)} = 1 \\ 
[10pt]
&=& \nabla_\theta \int p_\theta(x) dx 
		&\qquad \qquad \mathrel{\#} \text{exchange $\int$ and $\nabla$ can be exchanged} \\ 
[10pt]
&=& \nabla_{\theta} 1 
		&\qquad \qquad \mathrel{\#} \text{$p_\theta(x)$ is a density so} \int_x p_\theta(x) dx = 1 \\ 
[10pt]
&=& 0 
		&\qquad \qquad \mathrel{\#} \text{for c any constant}
\nabla c = 0
\end{array}
\end{equation*}
...
\bigskip
\noindent
So the mean $\mu$ of the score function,
$\mathbb{E}_{\theta}\big[\nabla_{\theta} \log p_{\theta}(X)\big]
= 0$. This condition (expected value is zero) is the
\emph{regularity condition} required by the Cram\'er Rao
Bound. See Section \ref{sec:crlb} for more on the Cram\'er Rao
Bound (sometimes Cram\'er Rao Lower Bound or CRLB).
...
&=& \dfrac{\nabla_{\theta}^{2} p_\theta(x)}{p_\theta(x)} - \dfrac{\nabla_\theta p_\theta(x)
\nabla_\theta p_\theta(x)^{\small{T}}}{{p_\theta(x)^2}} 
		&\mathrel{\#} \text{ finite differences}\\ 
[15pt]
...
p_\theta(x)^{\small{T}} 
		&\mathrel{\#} \nabla_{\theta} \log p_{\theta}(x) = 
		              \dfrac{\nabla_{\theta} p_\theta(x)}{p_\theta(x)} \\
[15pt]
&=& \dfrac{\nabla_{\theta}^{2} p_\theta(x)}{p_\theta(x)} -l_\theta l_{\theta}^{\small{T}} 
		&\mathrel{\#} \text{ definition of } l_\theta \\ 
[15pt]
&=& -\nabla_{\theta}^{2} \log p_\theta(x) + \dfrac{\nabla_{\theta}^{2} p_\theta(x)}{p_\theta(x)}
		&\mathrel{\#} l_\theta l_{\theta}^{\small{T}} = 
		              -\nabla_{\theta}^{2} \log p_\theta(x) + 
		              \dfrac{\nabla_{\theta}^{2} p_\theta(x)}{p_\theta(x)}
\end{array}
\end{equation*}
...
&=& \mathbb{E}_{x \sim p_\theta(x)} \big [l_\theta l_\theta^{\small{T}} \big ]
		&\qquad \mathrel{\#} \text{definition of } \mathbb{I}_\theta \\ 
[15pt]
&=& \mathbb{E}_\theta \Big [- \nabla_{\theta}^{2} \log p_\theta(x) +
\frac{\nabla_{\theta}^{2} p_\theta(x)}{p_\theta(x)} \Big ] 
		&\qquad \mathrel{\#} \text{ Equation (\ref{eqn:llt})} \\
[15pt] 
...
&=& - {\displaystyle \int p_\theta(x) \nabla^2 \log p_\theta(x) \; dx + 
					 \int \nabla^2 p_\theta(x) \; dx} 
		&\qquad \mathrel{\#} \dfrac{p_\theta(x)}{p_\theta(x)} = 1 \\ 
[15pt]
&=& -\mathbb{E}_{\theta} \big [\nabla^2 \log p_\theta(x) \big] + 
        			\nabla^2 \underbrace{\int p_\theta(x) \; dx}_{1}
		&\qquad \mathrel{\#} \text{$p_\theta(x)$ is a density} \\ 
[15pt]
&=& -\mathbb{E}_{\theta} \big [\nabla^2 \log p_\theta(x) \big] + \nabla^2 1 
		&\qquad \mathrel{\#} {\displaystyle \int p_\theta(x) \; dx = 1} \\ 
[18pt]
&=& -\mathbb{E}_{\theta} \big [\nabla^2 \log p_\theta(x) \big] + 0 
		&\qquad \mathrel{\#} \text{$\nabla c = 0$ for constant $c$} \\
[18pt]
&=&  -\mathbb{E}_{\theta} \big [\nabla^2 \log p_\theta(x) \big] 
		&\qquad \mathrel{\#} \mathbb{I}_\theta = -\mathbb{E}_{\theta} 
		                \big [\nabla^2 \log p_\theta(x) \big] 
...
\mathrm{Bernoulli}(p), p \in [0,1]$.  The standard formulation of
the Bernoulli probability mass function (pmf) $f$ for $x \in
[0,1]$ looks like
...
x) = p^{x}(1-p)^{1-x}$, then $\nabla P(X = x) = \frac{x}{p} - \frac{1-x}{1-p}$, 
so that

\begin{flalign}
\mathbb{I}_p &= \mathbb{E}_p \Bigg [\Bigg (\frac{X}{p} -
\Bigg(\frac{1-X}{1-p} \Bigg) \Bigg)^2\Bigg] \\ &=\frac{1}{p} +
...
\bigskip
\noindent
where $\mathbb{I}_p$ is the Fisher Information for  a single
sample. 
...
\begin{flalign}
\mathbb{E}\big [(X- \mu)^2 \big] &= \frac{1}{n} \mathrm{Var}(X)
\qquad \qquad \mathrel{\#} \mu = p \text{ (Bernoulli
disribution})\\ &= \frac{p(1-p)}{n} \; \qquad \qquad \mathrel{\#}
\text{Variance of the Bernoulli distribution} \\ &=
\frac{1}{\mathbb{I}_p} \cdot \frac{1}{n} \: \; \quad \qquad
\qquad \mathrel{\#} p(1-p) = \frac{1}{\mathbb{I}_p}, \text{
Equation } \ref{eqn:fisher} \\ \text{Var}(X)
&=\frac{1}{\mathbb{I}_{\theta}}
\label{eqn:inverse}
\end{flalign}

\bigskip
\noindent
It turns out that this inverse dependence of the variance on
Fisher Information (Equation \ref{eqn:inverse}) is unavoidable,
and is quantified by the Cram\'er Rao Bound, which provides lower
bounds on the mean squared error of all unbiased estimators.
...
won't review here, including observability\footnote{A statistic
is an observable random variable, which differentiates it both
from a parameter that is a generally unobservable quantity
describing a property of a statistical population, and from an
unobservable random variable, such as the difference between an
observed measurement and a population average, that is.,
$\frac{1}{n} \sum\limits_{i = 1}^n X_i - \mu$, where the $X_i$
are the observations and $\mu$ is the true population mean.}
completeness, consistency, sufficiency, unbiasedness.  Other
examples of properties of a statistic include minimum mean square
error, low variance, robustness, and computational convenience,
and measures of information such as the Fisher Information.
...
\nabla_{\tiny{\text{A}}} \ln p(x[0];\text{A}) =
...
\begin{flalign}
\sigma^2_{\text{min}} = - \frac{1}{\frac{\partial^2 \ln
...
\begin{equation*}
\text{MVUE} = \frac{1}{- \mathbb{E} \Big [\frac{\partial^2 \ln
...
p(x;\theta)} \Big [ \frac{\partial \ln p(x; \theta)}{\partial
...
[\frac{\partial^2 \ln p(x; \theta)}{\partial^2 \theta} \Big ]} 
\end{equation}
...
\mathbb{E} \Big [\frac{\partial^2 \ln p(x; \theta)}{\partial^2
...
\begin{flalign*}
\ln p(x;A) &= -\ln \big [(2 \pi \sigma^2)^{\frac{N}{2}} \big] - 
...
\theta)} \Bigg [ \frac{\partial \ln p(x;\theta)}{\partial
